<?php
		/**
		 * This controller facilitates the access of table spo_yoast_seo_links
		 *
		 * @package    HOF\controller\spo_yoast_seo_links
		 * @version    1.0
		 ...
		 * @modification 
		 */
class Spo_yoast_seo_links extends CI_Controller
{
    ...
    function __construct()
	{
            ...
            $this->load->model('spo_yoast_seo_links_model');
		...
		$this->log_create->init();
            ...
            if($search==true){ 
                ...
                foreach((count($_POST)>0?$_POST:$_GET) as $key => $val){ 
                   ...
                   $data['dataset'] = $this->spo_yoast_seo_links_model->get_data(array('search'=>$searchobj_array,'limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_yoast_seo_links.url'=>'Asc')));
            }else {
                   $data['dataset'] = $this->spo_yoast_seo_links_model->get_data(array('limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_yoast_seo_links.url'=>'Asc')));
            }   
            ...
            $wrapped_data['collist'] = $collist ? explode(':',$collist) : array('id','url','post_id','target_post_id','type')  ;
            ...
            $wrapped_data['widths'] = $widths ? $widths_arr : array('id'=>75,'url'=>75,'post_id'=>75,'target_post_id'=>75,'type'=>75) ;
		    ...
		    $wrapped_data['visibilityList'] = $visibilityList ? $visibilityList_arr : array('id'=>true,'url'=>true,'post_id'=>true,'target_post_id'=>true,'type'=>true);
	...
	function data_form($mode='add',$data = null){
		...
		if($mode != 'add'){
			...
			$row=$this->spo_yoast_seo_links_model->get_data(array('search'=>array(array('search_field' => 'id', 'search_text' => $data,'query_method'=>'equal')),'limit' => $limit,'offset'=>$offset)) ;
		...
		switch ($mode){
			...
			case "add": //-----------------------------------------------------
				...
				if ($this->input->post('url')) {
					if ($this->spo_yoast_seo_links_model->exists(array('url' => $this->input->post('url')))) {
						$message['msgText'] ='Sorry this record already exist!';
						$message['msgType'] ='failure';
						$message['focusField'] ='url';
					
					}else{
						$msg = $this->spo_yoast_seo_links_model->save($data2save,$key2save);
						if ($msg) {
			...
			case "edit":
				...
				if ($this->input->post('id')) {
					...
					if (!$this->spo_yoast_seo_links_model->exists(array('id' => $this->input->post('id')))) {
					 ...
					 $message['focusField'] ='url';
						...
						//checking existance
						if ($this->spo_yoast_seo_links_model->exists(array('url' => $this->input->post('url'),'id !=' => $this->input->post('id')))){
							$message['msgText'] ='Sorry this record already exist!';
							$message['msgType'] ='failure';
							$message['focusField'] ='url';
		...
		$wrapper['mode'] = $mode;
		$this->load->view('spo_yoast_seo_links_form',$wrapper);
	}
  	...
  	function save($action,$callback = null){
  		...
  		$save_data=array();
  		$key=array();
  		$metadata = $this->spo_yoast_seo_links_model->getMetadata();
  		...
  		if($this->AIR_call_flag){
  			...
  			*/  			  			
  			if($this->AR_Utilities->request_method == 'get'){
  				...
  				$added=$this->spo_yoast_seo_links_model->save($save_data,$key);  				
    ...
    function key_validate($action,$key) {
	    ...
	    $key_exist=$this->spo_yoast_seo_links_model->exists($key);
	...
	function delete($String)
	{
		...
		return($this->spo_yoast_seo_links_model->delete($delete));
	}
  	...
  	function loadForm($editable=null,$file='spo_yoast_seo_links_form') {
 	...
 	function loadWB($file='spo_yoast_seo_links_wb'){
	    ...
	    /**
 		 ...
 		 *
 		 *    http://.../index.php/Spo_yoast_seo_links/createDropdown/?sbv=null&sbt=null&format=html&multiple=false&sort=asc&filterby=status_equal=1
 		 *
 		 *
 		 *    1. For normal listing:
 		 *
 		 *    http://.../index.php/Spo_yoast_seo_links/createDropdown
 		 *
 		 *    2. For listing with multiple selection
 		 *
 		 *    http://.../index.php/Spo_yoast_seo_links/createDropdown/?multiple=true
 		 *
 		 *    3. For listing with pre selection (select by value) for single selection drop down
 		 *
 		 *    http://.../index.php/Spo_yoast_seo_links/createDropdown/?sbv=123
 		 *
 		 *    4. For listing with pre selection for multiple (select by value)
 		 *
 		 *    http://.../index.php/Spo_yoast_seo_links/createDropdown/?sbv=123,321,345&multiple=true
 		 *
 		 *    5. For listing with pre selection of select by text for single selection dropdown
 		 *
 		 *     http://.../index.php/Spo_yoast_seo_links/createDropdown/?sbt=test string&multiple=true
 		 *
 		 *    6. For xml(DHTMLX compatible) dropdown
 		 *
 		 *    http://.../index.php/Spo_yoast_seo_links/createDropdown/?format=xml
 		 *
 		 *    7. groupBy
 		 *    http://...../Spo_yoast_seo_links/createDropdown/?groupby=users.user_type
 		 *
 		 *	 8. groupCode
 		 *	  http://..../Spo_yoast_seo_links/createDropdown/?groupby=users.user_type&groupcodes=1=Staff,2=Admin,4=Manager
 		 *
 		 ...
 		 *
 		 *    groupBy - column to group, should be specified with the table name ex: spo_yoast_seo_links.staff_id
 		 *
	...
	public function createDropdown($data = false){
	  	...
	  	$isMultiple = null;
	  	$sort = null;
	    $sort_array = array('spo_yoast_seo_links.id' => 'ASC');
	  	$filterBy = null;
	   	$searchobj_array = null;
	   	$optionsOnly = false;
	   	$label = null;
	   	$displayClm = 'spo_yoast_seo_links.url';
	   	$groupBy = null;   
	   	$groupcodes = null;
	   	$nameTag = 'id'; 
	  	...
	  	if(is_array($data)){
	  		...
	  		$sort = isset($data['sort'])?$data['sort']:'spo_yoast_seo_links.id=ASC';
	  		...
	  		if(count($_POST)>0 || count($_GET)>0){
	        	...
	        	$myParams = count($_POST)>0? $_POST:$_GET;
	  			...
	  			$sort = isset($myParams['sort'])?$myParams['sort']:'spo_yoast_seo_links.id=ASC';
	      ...
	      $selection = 'spo_yoast_seo_links.id,spo_yoast_seo_links.url,spo_yoast_seo_links.post_id,spo_yoast_seo_links.target_post_id,spo_yoast_seo_links.type';
	    
	      // Fetch data from model
	  	  $dataset = $this->spo_yoast_seo_links_model->get_data(array(
