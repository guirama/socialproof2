<?php
		/**
		 * This controller facilitates the access of table spo_commentmeta
		 *
		 * @package    HOF\controller\spo_commentmeta
		 * @version    1.0
		 ...
		 * @modification 
		 */
class Spo_commentmeta extends CI_Controller
{
    ...
    function __construct()
	{
            ...
            $this->load->model('spo_commentmeta_model');
		...
		$this->log_create->init();
            ...
            if($search==true){ 
                ...
                foreach((count($_POST)>0?$_POST:$_GET) as $key => $val){ 
                   ...
                   $data['dataset'] = $this->spo_commentmeta_model->get_data(array('search'=>$searchobj_array,'limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_commentmeta.meta_key'=>'Asc')));
            }else {
                   $data['dataset'] = $this->spo_commentmeta_model->get_data(array('limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_commentmeta.meta_key'=>'Asc')));
            }   
            ...
            $wrapped_data['collist'] = $collist ? explode(':',$collist) : array('meta_id','comment_id','meta_key','meta_value')  ;
            ...
            $wrapped_data['widths'] = $widths ? $widths_arr : array('meta_id'=>75,'comment_id'=>75,'meta_key'=>75,'meta_value'=>75) ;
		    ...
		    $wrapped_data['visibilityList'] = $visibilityList ? $visibilityList_arr : array('meta_id'=>true,'comment_id'=>true,'meta_key'=>true,'meta_value'=>true);
		    ...
		    $wrapped_data['cellStyles'] = $cellStyles ? $stylesList_arr : array('','','text-align:left','text-align:left');
	...
	function data_form($mode='add',$data = null){
		...
		if($mode != 'add'){
			...
			$row=$this->spo_commentmeta_model->get_data(array('search'=>array(array('search_field' => 'meta_id', 'search_text' => $data,'query_method'=>'equal')),'limit' => $limit,'offset'=>$offset)) ;
		...
		//$postData = $this->input->post();
			...
			$key2save = array('meta_id' =>$this->input->post('meta_id'));
			}
		}
		...
		switch ($mode){
			...
			case "add": //-----------------------------------------------------
				...
				if ($this->input->post('meta_key')) {
					if ($this->spo_commentmeta_model->exists(array('meta_key' => $this->input->post('meta_key')))) {
						$message['msgText'] ='Sorry this record already exist!';
						$message['msgType'] ='failure';
						$message['focusField'] ='meta_key';
					
					}else{
						$msg = $this->spo_commentmeta_model->save($data2save,$key2save);
						if ($msg) {
			...
			case "edit":
				...
				if ($this->input->post('meta_id')) {
					...
					if (!$this->spo_commentmeta_model->exists(array('meta_id' => $this->input->post('meta_id')))) {
					 ...
					 $message['focusField'] ='meta_key';
						...
						//checking existance
						if ($this->spo_commentmeta_model->exists(array('meta_key' => $this->input->post('meta_key'),'meta_id !=' => $this->input->post('meta_id')))){
							$message['msgText'] ='Sorry this record already exist!';
							$message['msgType'] ='failure';
							$message['focusField'] ='meta_key';
		...
		$wrapper['mode'] = $mode;
		$this->load->view('spo_commentmeta_form',$wrapper);
	}
  	...
  	function save($action,$callback = null){
  		...
  		$save_data=array();
  		$key=array();
  		$metadata = $this->spo_commentmeta_model->getMetadata();
  		...
  		if($this->AIR_call_flag){
  			...
  			*/  			  			
  			if($this->AR_Utilities->request_method == 'get'){
  				...
  				$added=$this->spo_commentmeta_model->save($save_data,$key);  				
    ...
    function key_validate($action,$key) {
	    ...
	    $key_exist=$this->spo_commentmeta_model->exists($key);
	...
	function delete($String)
	{
		...
		return($this->spo_commentmeta_model->delete($delete));
	}
  	...
  	function loadForm($editable=null,$file='spo_commentmeta_form') {
 	...
 	function loadWB($file='spo_commentmeta_wb'){
	    ...
	    /**
 		 ...
 		 *
 		 *    http://.../index.php/Spo_commentmeta/createDropdown/?sbv=null&sbt=null&format=html&multiple=false&sort=asc&filterby=status_equal=1
 		 *
 		 *
 		 *    1. For normal listing:
 		 *
 		 *    http://.../index.php/Spo_commentmeta/createDropdown
 		 *
 		 *    2. For listing with multiple selection
 		 *
 		 *    http://.../index.php/Spo_commentmeta/createDropdown/?multiple=true
 		 *
 		 *    3. For listing with pre selection (select by value) for single selection drop down
 		 *
 		 *    http://.../index.php/Spo_commentmeta/createDropdown/?sbv=123
 		 *
 		 *    4. For listing with pre selection for multiple (select by value)
 		 *
 		 *    http://.../index.php/Spo_commentmeta/createDropdown/?sbv=123,321,345&multiple=true
 		 *
 		 *    5. For listing with pre selection of select by text for single selection dropdown
 		 *
 		 *     http://.../index.php/Spo_commentmeta/createDropdown/?sbt=test string&multiple=true
 		 *
 		 *    6. For xml(DHTMLX compatible) dropdown
 		 *
 		 *    http://.../index.php/Spo_commentmeta/createDropdown/?format=xml
 		 *
 		 *    7. groupBy
 		 *    http://...../Spo_commentmeta/createDropdown/?groupby=users.user_type
 		 *
 		 *	 8. groupCode
 		 *	  http://..../Spo_commentmeta/createDropdown/?groupby=users.user_type&groupcodes=1=Staff,2=Admin,4=Manager
 		 *
 		 ...
 		 *
 		 *    groupBy - column to group, should be specified with the table name ex: spo_commentmeta.staff_id
 		 *
	...
	public function createDropdown($data = false){
	  	...
	  	$isMultiple = null;
	  	$sort = null;
	    $sort_array = array('spo_commentmeta.meta_id' => 'ASC');
	  	$filterBy = null;
	   	$searchobj_array = null;
	   	$optionsOnly = false;
	   	$label = null;
	   	$displayClm = 'spo_commentmeta.meta_key';
	   	$groupBy = null;   
	   	$groupcodes = null;
	   	$nameTag = 'meta_id'; 
	  	...
	  	if(is_array($data)){
	  		...
	  		if(count($_POST)>0 || count($_GET)>0){
	        	...
	        	$myParams = count($_POST)>0? $_POST:$_GET;
	  			...
	  			$sort = isset($myParams['sort'])?$myParams['sort']:'spo_commentmeta.meta_id=ASC';
	  			...
	  			$nameTag = (!empty($nameTag))?$nameTag:'meta_id';                       
	      ...
	      $selection = 'spo_commentmeta.meta_id,spo_commentmeta.comment_id,spo_commentmeta.meta_key,spo_commentmeta.meta_value';
	    
	      // Fetch data from model
	  	  $dataset = $this->spo_commentmeta_model->get_data(array(
