<?php
		/**
		 * This controller facilitates the access of table spo_admins
		 *
		 * @package    HOF\controller\spo_admins
		 * @version    1.0
		 ...
		 * @modification 
		 */
class Spo_admins extends CI_Controller
{
    ...
    function __construct()
	{
            ...
            $this->load->model('spo_admins_model');
		...
		$this->log_create->init();
            ...
            if($search==true){ 
                ...
                foreach((count($_POST)>0?$_POST:$_GET) as $key => $val){ 
                   ...
                   $data['dataset'] = $this->spo_admins_model->get_data(array('search'=>$searchobj_array,'limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_admins.admin_name'=>'Asc')));
            }else {
                   $data['dataset'] = $this->spo_admins_model->get_data(array('limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_admins.admin_name'=>'Asc')));
            }   
            ...
            $wrapped_data['collist'] = $collist ? explode(':',$collist) : array('admin_id','admin_name','username','password','email')  ;
            ...
            $wrapped_data['widths'] = $widths ? $widths_arr : array('admin_id'=>75,'admin_name'=>75,'username'=>75,'password'=>75,'email'=>75) ;
		    ...
		    $wrapped_data['visibilityList'] = $visibilityList ? $visibilityList_arr : array('admin_id'=>true,'admin_name'=>true,'username'=>true,'password'=>true,'email'=>true);
		    ...
		    $wrapped_data['cellStyles'] = $cellStyles ? $stylesList_arr : array('text-align:center','text-align:left','text-align:left','text-align:left','text-align:left');
	...
	function data_form($mode='add',$data = null){
		...
		if($mode != 'add'){
			...
			$row=$this->spo_admins_model->get_data(array('search'=>array(array('search_field' => 'admin_id', 'search_text' => $data,'query_method'=>'equal')),'limit' => $limit,'offset'=>$offset)) ;
		...
		//$postData = $this->input->post();
			...
			$key2save = array('admin_id' =>$this->input->post('admin_id'));
			}
		}
		...
		switch ($mode){
			...
			case "add": //-----------------------------------------------------
				...
				if ($this->input->post('admin_name')) {
					if ($this->spo_admins_model->exists(array('admin_name' => $this->input->post('admin_name')))) {
						$message['msgText'] ='Sorry this record already exist!';
						$message['msgType'] ='failure';
						$message['focusField'] ='admin_name';
					
					}else{
						$msg = $this->spo_admins_model->save($data2save,$key2save);
						if ($msg) {
			...
			case "edit":
				...
				if ($this->input->post('admin_id')) {
					...
					if (!$this->spo_admins_model->exists(array('admin_id' => $this->input->post('admin_id')))) {
					 ...
					 $message['focusField'] ='admin_name';
						...
						//checking existance
						if ($this->spo_admins_model->exists(array('admin_name' => $this->input->post('admin_name'),'admin_id !=' => $this->input->post('admin_id')))){
							$message['msgText'] ='Sorry this record already exist!';
							$message['msgType'] ='failure';
							$message['focusField'] ='admin_name';
		...
		$wrapper['mode'] = $mode;
		$this->load->view('spo_admins_form',$wrapper);
	}
  	...
  	function save($action,$callback = null){
  		...
  		$save_data=array();
  		$key=array();
  		$metadata = $this->spo_admins_model->getMetadata();
  		...
  		if($this->AIR_call_flag){
  			...
  			*/  			  			
  			if($this->AR_Utilities->request_method == 'get'){
  				...
  				$added=$this->spo_admins_model->save($save_data,$key);  				
    ...
    function key_validate($action,$key) {
	    ...
	    $key_exist=$this->spo_admins_model->exists($key);
	...
	function delete($String)
	{
		...
		return($this->spo_admins_model->delete($delete));
	}
  	...
  	function loadForm($editable=null,$file='spo_admins_form') {
 	...
 	function loadWB($file='spo_admins_wb'){
	    ...
	    /**
 		 ...
 		 *
 		 *    http://.../index.php/Spo_admins/createDropdown/?sbv=null&sbt=null&format=html&multiple=false&sort=asc&filterby=status_equal=1
 		 *
 		 *
 		 *    1. For normal listing:
 		 *
 		 *    http://.../index.php/Spo_admins/createDropdown
 		 *
 		 *    2. For listing with multiple selection
 		 *
 		 *    http://.../index.php/Spo_admins/createDropdown/?multiple=true
 		 *
 		 *    3. For listing with pre selection (select by value) for single selection drop down
 		 *
 		 *    http://.../index.php/Spo_admins/createDropdown/?sbv=123
 		 *
 		 *    4. For listing with pre selection for multiple (select by value)
 		 *
 		 *    http://.../index.php/Spo_admins/createDropdown/?sbv=123,321,345&multiple=true
 		 *
 		 *    5. For listing with pre selection of select by text for single selection dropdown
 		 *
 		 *     http://.../index.php/Spo_admins/createDropdown/?sbt=test string&multiple=true
 		 *
 		 *    6. For xml(DHTMLX compatible) dropdown
 		 *
 		 *    http://.../index.php/Spo_admins/createDropdown/?format=xml
 		 *
 		 *    7. groupBy
 		 *    http://...../Spo_admins/createDropdown/?groupby=users.user_type
 		 *
 		 *	 8. groupCode
 		 *	  http://..../Spo_admins/createDropdown/?groupby=users.user_type&groupcodes=1=Staff,2=Admin,4=Manager
 		 *
 		 ...
 		 *
 		 *    groupBy - column to group, should be specified with the table name ex: spo_admins.staff_id
 		 *
	...
	public function createDropdown($data = false){
	  	...
	  	$isMultiple = null;
	  	$sort = null;
	    $sort_array = array('spo_admins.admin_id' => 'ASC');
	  	$filterBy = null;
	   	$searchobj_array = null;
	   	$optionsOnly = false;
	   	$label = null;
	   	$displayClm = 'spo_admins.admin_name';
	   	$groupBy = null;   
	   	$groupcodes = null;
	   	$nameTag = 'admin_id'; 
	  	...
	  	if(is_array($data)){
	  		...
	  		if(count($_POST)>0 || count($_GET)>0){
	        	...
	        	$myParams = count($_POST)>0? $_POST:$_GET;
	  			...
	  			$sort = isset($myParams['sort'])?$myParams['sort']:'spo_admins.admin_id=ASC';
	  			...
	  			$nameTag = (!empty($nameTag))?$nameTag:'admin_id';                       
	      ...
	      $selection = 'spo_admins.admin_id,spo_admins.admin_name,spo_admins.username,spo_admins.password,spo_admins.email';
	    
	      // Fetch data from model
	  	  $dataset = $this->spo_admins_model->get_data(array(
