<?php
		/**
		 * This controller facilitates the access of table spo_posts
		 *
		 * @package    HOF\controller\spo_posts
		 * @version    1.0
		 ...
		 * @modification 
		 */
class Spo_posts extends CI_Controller
{
    ...
    function __construct()
	{
            ...
            $this->load->model('spo_posts_model');
		...
		$this->log_create->init();
            ...
            if($search==true){ 
                ...
                foreach((count($_POST)>0?$_POST:$_GET) as $key => $val){ 
                   ...
                   $data['dataset'] = $this->spo_posts_model->get_data(array('search'=>$searchobj_array,'limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_posts.post_status'=>'Asc')));
            }else {
                   $data['dataset'] = $this->spo_posts_model->get_data(array('limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_posts.post_status'=>'Asc')));
            }   
            ...
            $wrapped_data['collist'] = $collist ? explode(':',$collist) : array('ID','post_author','post_date','post_date_gmt','post_content','post_title','post_excerpt','post_status','comment_status','ping_status','post_password','post_name','to_ping','pinged','post_modified','post_modified_gmt','post_content_filtered','post_parent','guid','menu_order','post_type','post_mime_type','comment_count')  ;
            ...
            $wrapped_data['widths'] = $widths ? $widths_arr : array('ID'=>75,'post_author'=>75,'post_date'=>75,'post_date_gmt'=>75,'post_content'=>75,'post_title'=>75,'post_excerpt'=>75,'post_status'=>75,'comment_status'=>75,'ping_status'=>75,'post_password'=>75,'post_name'=>75,'to_ping'=>75,'pinged'=>75,'post_modified'=>75,'post_modified_gmt'=>75,'post_content_filtered'=>75,'post_parent'=>75,'guid'=>75,'menu_order'=>75,'post_type'=>75,'post_mime_type'=>75,'comment_count'=>75) ;
		    ...
		    $wrapped_data['visibilityList'] = $visibilityList ? $visibilityList_arr : array('ID'=>true,'post_author'=>true,'post_date'=>true,'post_date_gmt'=>true,'post_content'=>true,'post_title'=>true,'post_excerpt'=>true,'post_status'=>true,'comment_status'=>true,'ping_status'=>true,'post_password'=>true,'post_name'=>true,'to_ping'=>true,'pinged'=>true,'post_modified'=>true,'post_modified_gmt'=>true,'post_content_filtered'=>true,'post_parent'=>true,'guid'=>true,'menu_order'=>true,'post_type'=>true,'post_mime_type'=>true,'comment_count'=>true);
		    ...
		    $wrapped_data['cellStyles'] = $cellStyles ? $stylesList_arr : array('','','text-align:center','text-align:center','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:center','text-align:center','text-align:left','text-align:left','text-align:left','text-align:center','text-align:left','text-align:left','text-align:left');
	...
	function data_form($mode='add',$data = null){
		...
		if($mode != 'add'){
			...
			$row=$this->spo_posts_model->get_data(array('search'=>array(array('search_field' => 'ID', 'search_text' => $data,'query_method'=>'equal')),'limit' => $limit,'offset'=>$offset)) ;
		...
		switch ($mode){
			...
			case "add": //-----------------------------------------------------
				...
				if ($this->input->post('post_status')) {
					if ($this->spo_posts_model->exists(array('post_status' => $this->input->post('post_status')))) {
						$message['msgText'] ='Sorry this record already exist!';
						$message['msgType'] ='failure';
						$message['focusField'] ='post_status';
					
					}else{
						$msg = $this->spo_posts_model->save($data2save,$key2save);
						if ($msg) {
			...
			case "edit":
				...
				if ($this->input->post('ID')) {
					...
					if (!$this->spo_posts_model->exists(array('ID' => $this->input->post('ID')))) {
					 ...
					 $message['focusField'] ='post_status';
						...
						//checking existance
						if ($this->spo_posts_model->exists(array('post_status' => $this->input->post('post_status'),'ID !=' => $this->input->post('ID')))){
							$message['msgText'] ='Sorry this record already exist!';
							$message['msgType'] ='failure';
							$message['focusField'] ='post_status';
		...
		$wrapper['mode'] = $mode;
		$this->load->view('spo_posts_form',$wrapper);
	}
  	...
  	function save($action,$callback = null){
  		...
  		$save_data=array();
  		$key=array();
  		$metadata = $this->spo_posts_model->getMetadata();
  		...
  		if($this->AIR_call_flag){
  			...
  			*/  			  			
  			if($this->AR_Utilities->request_method == 'get'){
  				...
  				$added=$this->spo_posts_model->save($save_data,$key);  				
    ...
    function key_validate($action,$key) {
	    ...
	    $key_exist=$this->spo_posts_model->exists($key);
	...
	function delete($String)
	{
		...
		return($this->spo_posts_model->delete($delete));
	}
  	...
  	function loadForm($editable=null,$file='spo_posts_form') {
 	...
 	function loadWB($file='spo_posts_wb'){
	    ...
	    /**
 		 ...
 		 *
 		 *    http://.../index.php/Spo_posts/createDropdown/?sbv=null&sbt=null&format=html&multiple=false&sort=asc&filterby=status_equal=1
 		 *
 		 *
 		 *    1. For normal listing:
 		 *
 		 *    http://.../index.php/Spo_posts/createDropdown
 		 *
 		 *    2. For listing with multiple selection
 		 *
 		 *    http://.../index.php/Spo_posts/createDropdown/?multiple=true
 		 *
 		 *    3. For listing with pre selection (select by value) for single selection drop down
 		 *
 		 *    http://.../index.php/Spo_posts/createDropdown/?sbv=123
 		 *
 		 *    4. For listing with pre selection for multiple (select by value)
 		 *
 		 *    http://.../index.php/Spo_posts/createDropdown/?sbv=123,321,345&multiple=true
 		 *
 		 *    5. For listing with pre selection of select by text for single selection dropdown
 		 *
 		 *     http://.../index.php/Spo_posts/createDropdown/?sbt=test string&multiple=true
 		 *
 		 *    6. For xml(DHTMLX compatible) dropdown
 		 *
 		 *    http://.../index.php/Spo_posts/createDropdown/?format=xml
 		 *
 		 *    7. groupBy
 		 *    http://...../Spo_posts/createDropdown/?groupby=users.user_type
 		 *
 		 *	 8. groupCode
 		 *	  http://..../Spo_posts/createDropdown/?groupby=users.user_type&groupcodes=1=Staff,2=Admin,4=Manager
 		 *
 		 ...
 		 *
 		 *    groupBy - column to group, should be specified with the table name ex: spo_posts.staff_id
 		 *
	...
	public function createDropdown($data = false){
	  	...
	  	$isMultiple = null;
	  	$sort = null;
	    $sort_array = array('spo_posts.ID' => 'ASC');
	  	$filterBy = null;
	   	$searchobj_array = null;
	   	$optionsOnly = false;
	   	$label = null;
	   	$displayClm = 'spo_posts.post_status';
	   	$groupBy = null;   
	   	$groupcodes = null;
	   	$nameTag = 'ID'; 
	  	...
	  	if(is_array($data)){
	  		...
	  		$sort = isset($data['sort'])?$data['sort']:'spo_posts.ID=ASC';
	  		...
	  		if(count($_POST)>0 || count($_GET)>0){
	        	...
	        	$myParams = count($_POST)>0? $_POST:$_GET;
	  			...
	  			$sort = isset($myParams['sort'])?$myParams['sort']:'spo_posts.ID=ASC';
