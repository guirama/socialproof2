<?php
		 ...
		 *
		 * @package    HOF\controller\spo_yoast_seo_meta
		 * @version    1.0
		 ...
		 * @modification 
		 */
class Spo_yoast_seo_meta extends CI_Controller
{
    ...
    function __construct()
	{
            ...
            $this->load->model('spo_yoast_seo_meta_model');
		...
		$this->log_create->init();
            ...
            if($search==true){ 
                ...
                foreach((count($_POST)>0?$_POST:$_GET) as $key => $val){ 
                   ...
                   $data['dataset'] = $this->spo_yoast_seo_meta_model->get_data(array('search'=>$searchobj_array,'limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_yoast_seo_meta.incoming_link_count'=>'Asc')));
            }else {
                   $data['dataset'] = $this->spo_yoast_seo_meta_model->get_data(array('limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_yoast_seo_meta.incoming_link_count'=>'Asc')));
            }   
            ...
            $wrapped_data['collist'] = $collist ? explode(':',$collist) : array('object_id','internal_link_count','incoming_link_count')  ;
            ...
            $wrapped_data['widths'] = $widths ? $widths_arr : array('object_id'=>75,'internal_link_count'=>75,'incoming_link_count'=>75) ;
		    ...
		    $wrapped_data['visibilityList'] = $visibilityList ? $visibilityList_arr : array('object_id'=>true,'internal_link_count'=>true,'incoming_link_count'=>true);
		    ...
		    $wrapped_data['cellStyles'] = $cellStyles ? $stylesList_arr : array('','text-align:center','text-align:center');
	...
	function data_form($mode='add',$data = null){
		...
		if($mode != 'add'){
			...
			$row=$this->spo_yoast_seo_meta_model->get_data(array('search'=>array(array('search_field' => 'object_id', 'search_text' => $data,'query_method'=>'equal')),'limit' => $limit,'offset'=>$offset)) ;
		...
		//$postData = $this->input->post();
			...
			$key2save = array('object_id' =>$this->input->post('object_id'));
			}
		}
		...
		switch ($mode){
			...
			case "add": //-----------------------------------------------------
				...
				//Check duplicates
				if ($this->input->post('incoming_link_count')) {
					if ($this->spo_yoast_seo_meta_model->exists(array('incoming_link_count' => $this->input->post('incoming_link_count')))) {
						$message['msgText'] ='Sorry this record already exist!';
						$message['msgType'] ='failure';
						$message['focusField'] ='incoming_link_count';
					
					}else{
						$msg = $this->spo_yoast_seo_meta_model->save($data2save,$key2save);
						if ($msg) {
			...
			case "edit":
				...
				if ($this->input->post('object_id')) {
					...
					if (!$this->spo_yoast_seo_meta_model->exists(array('object_id' => $this->input->post('object_id')))) {
					 ...
					 $message['focusField'] ='incoming_link_count';
						...
						//checking existance
						if ($this->spo_yoast_seo_meta_model->exists(array('incoming_link_count' => $this->input->post('incoming_link_count'),'object_id !=' => $this->input->post('object_id')))){
							$message['msgText'] ='Sorry this record already exist!';
							$message['msgType'] ='failure';
							$message['focusField'] ='incoming_link_count';
		...
		$wrapper['mode'] = $mode;
		$this->load->view('spo_yoast_seo_meta_form',$wrapper);
	}
  	...
  	function save($action,$callback = null){
  		...
  		$save_data=array();
  		$key=array();
  		$metadata = $this->spo_yoast_seo_meta_model->getMetadata();
  		...
  		if($this->AIR_call_flag){
  			...
  			*/  			  			
  			if($this->AR_Utilities->request_method == 'get'){
  				...
  				$added=$this->spo_yoast_seo_meta_model->save($save_data,$key);  				
    ...
    function key_validate($action,$key) {
	    ...
	    $key_exist=$this->spo_yoast_seo_meta_model->exists($key);
	...
	function delete($String)
	{
		...
		return($this->spo_yoast_seo_meta_model->delete($delete));
	}
  	...
  	function loadForm($editable=null,$file='spo_yoast_seo_meta_form') {
 	...
 	function loadWB($file='spo_yoast_seo_meta_wb'){
	    ...
	    /**
 		 ...
 		 *
 		 *    http://.../index.php/Spo_yoast_seo_meta/createDropdown/?sbv=null&sbt=null&format=html&multiple=false&sort=asc&filterby=status_equal=1
 		 *
 		 *
 		 *    1. For normal listing:
 		 *
 		 *    http://.../index.php/Spo_yoast_seo_meta/createDropdown
 		 *
 		 *    2. For listing with multiple selection
 		 *
 		 *    http://.../index.php/Spo_yoast_seo_meta/createDropdown/?multiple=true
 		 *
 		 *    3. For listing with pre selection (select by value) for single selection drop down
 		 *
 		 *    http://.../index.php/Spo_yoast_seo_meta/createDropdown/?sbv=123
 		 *
 		 *    4. For listing with pre selection for multiple (select by value)
 		 *
 		 *    http://.../index.php/Spo_yoast_seo_meta/createDropdown/?sbv=123,321,345&multiple=true
 		 *
 		 *    5. For listing with pre selection of select by text for single selection dropdown
 		 *
 		 *     http://.../index.php/Spo_yoast_seo_meta/createDropdown/?sbt=test string&multiple=true
 		 *
 		 *    6. For xml(DHTMLX compatible) dropdown
 		 *
 		 *    http://.../index.php/Spo_yoast_seo_meta/createDropdown/?format=xml
 		 *
 		 *    7. groupBy
 		 *    http://...../Spo_yoast_seo_meta/createDropdown/?groupby=users.user_type
 		 *
 		 *	 8. groupCode
 		 *	  http://..../Spo_yoast_seo_meta/createDropdown/?groupby=users.user_type&groupcodes=1=Staff,2=Admin,4=Manager
 		 *
 		 ...
 		 *
 		 *    groupBy - column to group, should be specified with the table name ex: spo_yoast_seo_meta.staff_id
 		 *
	...
	public function createDropdown($data = false){
	  	...
	  	$isMultiple = null;
	  	$sort = null;
	    $sort_array = array('spo_yoast_seo_meta.object_id' => 'ASC');
	  	$filterBy = null;
	   	$searchobj_array = null;
	   	$optionsOnly = false;
	   	$label = null;
	   	$displayClm = 'spo_yoast_seo_meta.incoming_link_count';
	   	$groupBy = null;   
	   	$groupcodes = null;
	   	$nameTag = 'object_id'; 
	  	...
	  	if(is_array($data)){
	  		...
	  		$sort = isset($data['sort'])?$data['sort']:'spo_yoast_seo_meta.object_id=ASC';
	  		...
	  		if(count($_POST)>0 || count($_GET)>0){
	        	...
	        	$myParams = count($_POST)>0? $_POST:$_GET;
	  			...
	  			$sort = isset($myParams['sort'])?$myParams['sort']:'spo_yoast_seo_meta.object_id=ASC';
	      ...
	      $selection = 'spo_yoast_seo_meta.object_id,spo_yoast_seo_meta.internal_link_count,spo_yoast_seo_meta.incoming_link_count';
	    
	      // Fetch data from model
	  	  $dataset = $this->spo_yoast_seo_meta_model->get_data(array(
