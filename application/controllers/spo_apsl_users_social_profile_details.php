<?php
		/**
		 * This controller facilitates the access of table spo_apsl_users_social_profile_details
		 *
		 * @package    HOF\controller\spo_apsl_users_social_profile_details
		 * @version    1.0
		 ...
		 * @modification 
		 */
class Spo_apsl_users_social_profile_details extends CI_Controller
{
    ...
    function __construct()
	{
            ...
            $this->load->model('spo_apsl_users_social_profile_details_model');
		...
		$this->log_create->init();
            ...
            if($search==true){ 
                ...
                foreach((count($_POST)>0?$_POST:$_GET) as $key => $val){ 
                   ...
                   $data['dataset'] = $this->spo_apsl_users_social_profile_details_model->get_data(array('search'=>$searchobj_array,'limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_apsl_users_social_profile_details.provider_name'=>'Asc')));
            }else {
                   $data['dataset'] = $this->spo_apsl_users_social_profile_details_model->get_data(array('limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_apsl_users_social_profile_details.provider_name'=>'Asc')));
            }   
            ...
            $wrapped_data['collist'] = $collist ? explode(':',$collist) : array('id','user_id','provider_name','identifier','unique_verifier','email','email_verified','first_name','last_name','profile_url','website_url','photo_url','display_name','description','gender','language','age','birthday','birthmonth','birthyear','phone','address','country','region','city','zip')  ;
            ...
            $wrapped_data['widths'] = $widths ? $widths_arr : array('id'=>75,'user_id'=>75,'provider_name'=>75,'identifier'=>75,'unique_verifier'=>75,'email'=>75,'email_verified'=>75,'first_name'=>75,'last_name'=>75,'profile_url'=>75,'website_url'=>75,'photo_url'=>75,'display_name'=>75,'description'=>75,'gender'=>75,'language'=>75,'age'=>75,'birthday'=>75,'birthmonth'=>75,'birthyear'=>75,'phone'=>75,'address'=>75,'country'=>75,'region'=>75,'city'=>75,'zip'=>75) ;
		    ...
		    $wrapped_data['visibilityList'] = $visibilityList ? $visibilityList_arr : array('id'=>true,'user_id'=>true,'provider_name'=>true,'identifier'=>true,'unique_verifier'=>true,'email'=>true,'email_verified'=>true,'first_name'=>true,'last_name'=>true,'profile_url'=>true,'website_url'=>true,'photo_url'=>true,'display_name'=>true,'description'=>true,'gender'=>true,'language'=>true,'age'=>true,'birthday'=>true,'birthmonth'=>true,'birthyear'=>true,'phone'=>true,'address'=>true,'country'=>true,'region'=>true,'city'=>true,'zip'=>true);
		    ...
		    $wrapped_data['cellStyles'] = $cellStyles ? $stylesList_arr : array('text-align:center','text-align:center','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:center','text-align:center','text-align:center','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left');
	...
	function data_form($mode='add',$data = null){
		...
		if($mode != 'add'){
			...
			$row=$this->spo_apsl_users_social_profile_details_model->get_data(array('search'=>array(array('search_field' => 'id', 'search_text' => $data,'query_method'=>'equal')),'limit' => $limit,'offset'=>$offset)) ;
		...
		//$postData = $this->input->post();
			...
			$key2save = array('id' =>$this->input->post('id'));
			}
		}
		...
		switch ($mode){
			...
			case "add": //-----------------------------------------------------
				...
				if ($this->input->post('provider_name')) {
					if ($this->spo_apsl_users_social_profile_details_model->exists(array('provider_name' => $this->input->post('provider_name')))) {
						$message['msgText'] ='Sorry this record already exist!';
						$message['msgType'] ='failure';
						$message['focusField'] ='provider_name';
					
					}else{
						$msg = $this->spo_apsl_users_social_profile_details_model->save($data2save,$key2save);
						if ($msg) {
			...
			case "edit":
				...
				if ($this->input->post('id')) {
					...
					if (!$this->spo_apsl_users_social_profile_details_model->exists(array('id' => $this->input->post('id')))) {
					 ...
					 $message['focusField'] ='provider_name';
						...
						//checking existance
						if ($this->spo_apsl_users_social_profile_details_model->exists(array('provider_name' => $this->input->post('provider_name'),'id !=' => $this->input->post('id')))){
							$message['msgText'] ='Sorry this record already exist!';
							$message['msgType'] ='failure';
							$message['focusField'] ='provider_name';
		...
		$wrapper['mode'] = $mode;
		$this->load->view('spo_apsl_users_social_profile_details_form',$wrapper);
	}
  	...
  	function save($action,$callback = null){
  		...
  		$save_data=array();
  		$key=array();
  		$metadata = $this->spo_apsl_users_social_profile_details_model->getMetadata();
  		...
  		if($this->AIR_call_flag){
  			...
  			*/  			  			
  			if($this->AR_Utilities->request_method == 'get'){
  				...
  				$added=$this->spo_apsl_users_social_profile_details_model->save($save_data,$key);  				
    ...
    function key_validate($action,$key) {
	    ...
	    $key_exist=$this->spo_apsl_users_social_profile_details_model->exists($key);
	...
	function delete($String)
	{
		...
		return($this->spo_apsl_users_social_profile_details_model->delete($delete));
	}
  	...
  	function loadForm($editable=null,$file='spo_apsl_users_social_profile_details_form') {
 	...
 	function loadWB($file='spo_apsl_users_social_profile_details_wb'){
	    ...
	    /**
 		 ...
 		 *
 		 *    http://.../index.php/Spo_apsl_users_social_profile_details/createDropdown/?sbv=null&sbt=null&format=html&multiple=false&sort=asc&filterby=status_equal=1
 		 *
 		 *
 		 *    1. For normal listing:
 		 *
 		 *    http://.../index.php/Spo_apsl_users_social_profile_details/createDropdown
 		 *
 		 *    2. For listing with multiple selection
 		 *
 		 *    http://.../index.php/Spo_apsl_users_social_profile_details/createDropdown/?multiple=true
 		 *
 		 *    3. For listing with pre selection (select by value) for single selection drop down
 		 *
 		 *    http://.../index.php/Spo_apsl_users_social_profile_details/createDropdown/?sbv=123
 		 *
 		 *    4. For listing with pre selection for multiple (select by value)
 		 *
 		 *    http://.../index.php/Spo_apsl_users_social_profile_details/createDropdown/?sbv=123,321,345&multiple=true
 		 *
 		 *    5. For listing with pre selection of select by text for single selection dropdown
 		 *
 		 *     http://.../index.php/Spo_apsl_users_social_profile_details/createDropdown/?sbt=test string&multiple=true
 		 *
 		 *    6. For xml(DHTMLX compatible) dropdown
 		 *
 		 *    http://.../index.php/Spo_apsl_users_social_profile_details/createDropdown/?format=xml
 		 *
 		 *    7. groupBy
 		 *    http://...../Spo_apsl_users_social_profile_details/createDropdown/?groupby=users.user_type
 		 *
 		 *	 8. groupCode
 		 *	  http://..../Spo_apsl_users_social_profile_details/createDropdown/?groupby=users.user_type&groupcodes=1=Staff,2=Admin,4=Manager
 		 *
 		 ...
 		 *
 		 *    groupBy - column to group, should be specified with the table name ex: spo_apsl_users_social_profile_details.staff_id
 		 *
	...
	public function createDropdown($data = false){
	  	...
	  	$isMultiple = null;
	  	$sort = null;
	    $sort_array = array('spo_apsl_users_social_profile_details.id' => 'ASC');
	  	$filterBy = null;
	   	$searchobj_array = null;
	   	$optionsOnly = false;
	   	$label = null;
	   	$displayClm = 'spo_apsl_users_social_profile_details.provider_name';
	   	$groupBy = null;   
	   	$groupcodes = null;
	   	$nameTag = 'id'; 
	  	...
	  	if(is_array($data)){
	  		...
	  		$sort = isset($data['sort'])?$data['sort']:'spo_apsl_users_social_profile_details.id=ASC';
	  		...
	  		$nameTag = isset($data['nameTag'])?$data['nameTag']:'id';    
	  		...
	  		if(count($_POST)>0 || count($_GET)>0){
	        	...
	        	$myParams = count($_POST)>0? $_POST:$_GET;
	  			...
	  			$sort = isset($myParams['sort'])?$myParams['sort']:'spo_apsl_users_social_profile_details.id=ASC';
	  			...
	  			$nameTag = (!empty($nameTag))?$nameTag:'id';                       
