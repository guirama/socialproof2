<?php
		/**
		 * This controller facilitates the access of table spo_usermeta
		 *
		 * @package    HOF\controller\spo_usermeta
		 * @version    1.0
		 ...
		 * @modification 
		 */
class Spo_usermeta extends CI_Controller
{
		...
		$this->log_create->init();
            ...
            if($search==true){ 
                ...
                foreach((count($_POST)>0?$_POST:$_GET) as $key => $val){ 
                   ...
                   $data['dataset'] = $this->spo_usermeta_model->get_data(array('search'=>$searchobj_array,'limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_usermeta.meta_key'=>'Asc')));
            }else {
                   $data['dataset'] = $this->spo_usermeta_model->get_data(array('limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_usermeta.meta_key'=>'Asc')));
            }   
            ...
            $wrapped_data['collist'] = $collist ? explode(':',$collist) : array('umeta_id','user_id','meta_key','meta_value')  ;
            ...
            $wrapped_data['widths'] = $widths ? $widths_arr : array('umeta_id'=>75,'user_id'=>75,'meta_key'=>75,'meta_value'=>75) ;
		    ...
		    $wrapped_data['visibilityList'] = $visibilityList ? $visibilityList_arr : array('umeta_id'=>true,'user_id'=>true,'meta_key'=>true,'meta_value'=>true);
	...
	function data_form($mode='add',$data = null){
		...
		if($mode != 'add'){
			...
			$row=$this->spo_usermeta_model->get_data(array('search'=>array(array('search_field' => 'umeta_id', 'search_text' => $data,'query_method'=>'equal')),'limit' => $limit,'offset'=>$offset)) ;
		...
		//$postData = $this->input->post();
			...
			if($this->input->post('umeta_id') !=''){
			$key2save = array('umeta_id' =>$this->input->post('umeta_id'));
			}
		}
		...
		switch ($mode){
			...
			case "add": //-----------------------------------------------------
				...
				if ($this->input->post('meta_key')) {
					if ($this->spo_usermeta_model->exists(array('meta_key' => $this->input->post('meta_key')))) {
						$message['msgText'] ='Sorry this record already exist!';
						$message['msgType'] ='failure';
						$message['focusField'] ='meta_key';
					
					}else{
						$msg = $this->spo_usermeta_model->save($data2save,$key2save);
						if ($msg) {
			...
			case "edit":
				...
				if ($this->input->post('umeta_id')) {
					...
					if (!$this->spo_usermeta_model->exists(array('umeta_id' => $this->input->post('umeta_id')))) {
					 ...
					 $message['focusField'] ='meta_key';
						...
						//checking existance
						if ($this->spo_usermeta_model->exists(array('meta_key' => $this->input->post('meta_key'),'umeta_id !=' => $this->input->post('umeta_id')))){
							$message['msgText'] ='Sorry this record already exist!';
							$message['msgType'] ='failure';
							$message['focusField'] ='meta_key';
		...
		$wrapper['mode'] = $mode;
		$this->load->view('spo_usermeta_form',$wrapper);
	}
  	...
  	function save($action,$callback = null){
  		...
  		$save_data=array();
  		$key=array();
  		$metadata = $this->spo_usermeta_model->getMetadata();
  		...
  		if($this->AIR_call_flag){
  			...
  			*/  			  			
  			if($this->AR_Utilities->request_method == 'get'){
  				...
  				$added=$this->spo_usermeta_model->save($save_data,$key);  				
    ...
    function key_validate($action,$key) {
	    ...
	    $key_exist=$this->spo_usermeta_model->exists($key);
	...
	function delete($String)
	{
		...
		return($this->spo_usermeta_model->delete($delete));
	}
  	...
  	function loadForm($editable=null,$file='spo_usermeta_form') {
 	...
 	function loadWB($file='spo_usermeta_wb'){
	    ...
	    /**
 		 ...
 		 *
 		 *    http://.../index.php/Spo_usermeta/createDropdown/?sbv=null&sbt=null&format=html&multiple=false&sort=asc&filterby=status_equal=1
 		 *
 		 *
 		 *    1. For normal listing:
 		 *
 		 *    http://.../index.php/Spo_usermeta/createDropdown
 		 *
 		 *    2. For listing with multiple selection
 		 *
 		 *    http://.../index.php/Spo_usermeta/createDropdown/?multiple=true
 		 *
 		 *    3. For listing with pre selection (select by value) for single selection drop down
 		 *
 		 *    http://.../index.php/Spo_usermeta/createDropdown/?sbv=123
 		 *
 		 *    4. For listing with pre selection for multiple (select by value)
 		 *
 		 *    http://.../index.php/Spo_usermeta/createDropdown/?sbv=123,321,345&multiple=true
 		 *
 		 *    5. For listing with pre selection of select by text for single selection dropdown
 		 *
 		 *     http://.../index.php/Spo_usermeta/createDropdown/?sbt=test string&multiple=true
 		 *
 		 *    6. For xml(DHTMLX compatible) dropdown
 		 *
 		 *    http://.../index.php/Spo_usermeta/createDropdown/?format=xml
 		 *
 		 *    7. groupBy
 		 *    http://...../Spo_usermeta/createDropdown/?groupby=users.user_type
 		 *
 		 *	 8. groupCode
 		 *	  http://..../Spo_usermeta/createDropdown/?groupby=users.user_type&groupcodes=1=Staff,2=Admin,4=Manager
 		 *
 		 ...
 		 *
 		 *    groupBy - column to group, should be specified with the table name ex: spo_usermeta.staff_id
 		 *
	...
	public function createDropdown($data = false){
	  	...
	  	$isMultiple = null;
	  	$sort = null;
	    $sort_array = array('spo_usermeta.umeta_id' => 'ASC');
	  	$filterBy = null;
	   	$searchobj_array = null;
	   	$optionsOnly = false;
	   	$label = null;
	   	$displayClm = 'spo_usermeta.meta_key';
	   	$groupBy = null;   
	   	$groupcodes = null;
	   	$nameTag = 'umeta_id'; 
	  	...
	  	if(is_array($data)){
	  		...
	  		if(count($_POST)>0 || count($_GET)>0){
	        	...
	        	$myParams = count($_POST)>0? $_POST:$_GET;
	  			...
	  			$sort = isset($myParams['sort'])?$myParams['sort']:'spo_usermeta.umeta_id=ASC';
	  			...
	  			$nameTag = (!empty($nameTag))?$nameTag:'umeta_id';                       
	      ...
	      $selection = 'spo_usermeta.umeta_id,spo_usermeta.user_id,spo_usermeta.meta_key,spo_usermeta.meta_value';
	    
	      // Fetch data from model
	  	  $dataset = $this->spo_usermeta_model->get_data(array(
