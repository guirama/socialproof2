<?php
		/**
		 * This controller facilitates the access of table spo_popup_reviews
		 *
		 * @package    HOF\controller\spo_popup_reviews
		 * @version    1.0
		 ...
		 * @modification 
		 */
class Spo_popup_reviews extends CI_Controller
{
    ...
    function __construct()
	{
            ...
            $this->load->model('spo_popup_reviews_model');
		...
		$this->log_create->init();
            ...
            if($search==true){ 
                ...
                foreach((count($_POST)>0?$_POST:$_GET) as $key => $val){ 
                   ...
                   $data['dataset'] = $this->spo_popup_reviews_model->get_data(array('search'=>$searchobj_array,'limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_popup_reviews.id'=>'Asc')));
            }else {
                   $data['dataset'] = $this->spo_popup_reviews_model->get_data(array('limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_popup_reviews.id'=>'Asc')));
            }   
            ...
            $wrapped_data['collist'] = $collist ? explode(':',$collist) : array('review_id','id','user_id','rating','reviewer_name','reviewer_email','reviewer_image','review','provider_name','profile_link','created_on','approved')  ;
            ...
            $wrapped_data['widths'] = $widths ? $widths_arr : array('review_id'=>75,'id'=>75,'user_id'=>75,'rating'=>75,'reviewer_name'=>75,'reviewer_email'=>75,'reviewer_image'=>75,'review'=>75,'provider_name'=>75,'profile_link'=>75,'created_on'=>75,'approved'=>75) ;
		    ...
		    $wrapped_data['visibilityList'] = $visibilityList ? $visibilityList_arr : array('review_id'=>true,'id'=>true,'user_id'=>true,'rating'=>true,'reviewer_name'=>true,'reviewer_email'=>true,'reviewer_image'=>true,'review'=>true,'provider_name'=>true,'profile_link'=>true,'created_on'=>true,'approved'=>true);
		    ...
		    $wrapped_data['cellStyles'] = $cellStyles ? $stylesList_arr : array('text-align:center','text-align:left','text-align:center','text-align:center','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:center','text-align:center');
	...
	function data_form($mode='add',$data = null){
		...
		if($mode != 'add'){
			...
			$row=$this->spo_popup_reviews_model->get_data(array('search'=>array(array('search_field' => 'review_id', 'search_text' => $data,'query_method'=>'equal')),'limit' => $limit,'offset'=>$offset)) ;
		...
		//$postData = $this->input->post();
			...
			$key2save = array('review_id' =>$this->input->post('review_id'));
			}
		}
		...
		switch ($mode){
			...
			case "add": //-----------------------------------------------------
				...
				if ($this->input->post('id')) {
					if ($this->spo_popup_reviews_model->exists(array('id' => $this->input->post('id')))) {
						$message['msgText'] ='Sorry this record already exist!';
						$message['msgType'] ='failure';
						$message['focusField'] ='id';
					
					}else{
						$msg = $this->spo_popup_reviews_model->save($data2save,$key2save);
						if ($msg) {
			...
			case "edit":
				...
				if ($this->input->post('review_id')) {
					...
					if (!$this->spo_popup_reviews_model->exists(array('review_id' => $this->input->post('review_id')))) {
					 ...
					 $message['focusField'] ='id';
						...
						//checking existance
						if ($this->spo_popup_reviews_model->exists(array('id' => $this->input->post('id'),'review_id !=' => $this->input->post('review_id')))){
							$message['msgText'] ='Sorry this record already exist!';
							$message['msgType'] ='failure';
							$message['focusField'] ='id';
		...
		$wrapper['mode'] = $mode;
		$this->load->view('spo_popup_reviews_form',$wrapper);
	}
  	...
  	function save($action,$callback = null){
  		...
  		$save_data=array();
  		$key=array();
  		$metadata = $this->spo_popup_reviews_model->getMetadata();
  		...
  		if($this->AIR_call_flag){
  			...
  			*/  			  			
  			if($this->AR_Utilities->request_method == 'get'){
  				...
  				$added=$this->spo_popup_reviews_model->save($save_data,$key);  				
    ...
    function key_validate($action,$key) {
	    ...
	    $key_exist=$this->spo_popup_reviews_model->exists($key);
	...
	function delete($String)
	{
		...
		return($this->spo_popup_reviews_model->delete($delete));
	}
  	...
  	function loadForm($editable=null,$file='spo_popup_reviews_form') {
 	...
 	function loadWB($file='spo_popup_reviews_wb'){
	    ...
	    /**
 		 ...
 		 *
 		 *    http://.../index.php/Spo_popup_reviews/createDropdown/?sbv=null&sbt=null&format=html&multiple=false&sort=asc&filterby=status_equal=1
 		 *
 		 *
 		 *    1. For normal listing:
 		 *
 		 *    http://.../index.php/Spo_popup_reviews/createDropdown
 		 *
 		 *    2. For listing with multiple selection
 		 *
 		 *    http://.../index.php/Spo_popup_reviews/createDropdown/?multiple=true
 		 *
 		 *    3. For listing with pre selection (select by value) for single selection drop down
 		 *
 		 *    http://.../index.php/Spo_popup_reviews/createDropdown/?sbv=123
 		 *
 		 *    4. For listing with pre selection for multiple (select by value)
 		 *
 		 *    http://.../index.php/Spo_popup_reviews/createDropdown/?sbv=123,321,345&multiple=true
 		 *
 		 *    5. For listing with pre selection of select by text for single selection dropdown
 		 *
 		 *     http://.../index.php/Spo_popup_reviews/createDropdown/?sbt=test string&multiple=true
 		 *
 		 *    6. For xml(DHTMLX compatible) dropdown
 		 *
 		 *    http://.../index.php/Spo_popup_reviews/createDropdown/?format=xml
 		 *
 		 *    7. groupBy
 		 *    http://...../Spo_popup_reviews/createDropdown/?groupby=users.user_type
 		 *
 		 *	 8. groupCode
 		 *	  http://..../Spo_popup_reviews/createDropdown/?groupby=users.user_type&groupcodes=1=Staff,2=Admin,4=Manager
 		 *
 		 ...
 		 *
 		 *    groupBy - column to group, should be specified with the table name ex: spo_popup_reviews.staff_id
 		 *
	...
	public function createDropdown($data = false){
	  	...
	  	$isMultiple = null;
	  	$sort = null;
	    $sort_array = array('spo_popup_reviews.review_id' => 'ASC');
	  	$filterBy = null;
	   	$searchobj_array = null;
	   	$optionsOnly = false;
	   	$label = null;
	   	$displayClm = 'spo_popup_reviews.id';
	   	$groupBy = null;   
	   	$groupcodes = null;
	   	$nameTag = 'review_id'; 
	  	...
	  	if(is_array($data)){
	  		...
	  		if(count($_POST)>0 || count($_GET)>0){
	        	...
	        	$myParams = count($_POST)>0? $_POST:$_GET;
	  			...
	  			$sort = isset($myParams['sort'])?$myParams['sort']:'spo_popup_reviews.review_id=ASC';
	  			...
	  			$nameTag = (!empty($nameTag))?$nameTag:'review_id';                       
	      ...
	      $selection = 'spo_popup_reviews.review_id,spo_popup_reviews.id,spo_popup_reviews.user_id,spo_popup_reviews.rating,spo_popup_reviews.reviewer_name,spo_popup_reviews.reviewer_email,spo_popup_reviews.reviewer_image,spo_popup_reviews.review,spo_popup_reviews.provider_name,spo_popup_reviews.profile_link,spo_popup_reviews.created_on,spo_popup_reviews.approved';
	    
	      // Fetch data from model
	  	  $dataset = $this->spo_popup_reviews_model->get_data(array(
