<?php
		/**
		 * This controller facilitates the access of table spo_options
		 *
		 * @package    HOF\controller\spo_options
		 * @version    1.0
		 ...
		 * @modification 
		 */
class Spo_options extends CI_Controller
{
    ...
    function __construct()
	{
            ...
            $this->load->model('spo_options_model');
		...
		$this->log_create->init();
            ...
            if($search==true){ 
                ...
                foreach((count($_POST)>0?$_POST:$_GET) as $key => $val){ 
                   ...
                   $data['dataset'] = $this->spo_options_model->get_data(array('search'=>$searchobj_array,'limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_options.option_name'=>'Asc')));
            }else {
                   $data['dataset'] = $this->spo_options_model->get_data(array('limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_options.option_name'=>'Asc')));
            }   
            ...
            $wrapped_data['collist'] = $collist ? explode(':',$collist) : array('option_id','option_name','option_value','autoload')  ;
            ...
            $wrapped_data['widths'] = $widths ? $widths_arr : array('option_id'=>75,'option_name'=>75,'option_value'=>75,'autoload'=>75) ;
		    ...
		    $wrapped_data['visibilityList'] = $visibilityList ? $visibilityList_arr : array('option_id'=>true,'option_name'=>true,'option_value'=>true,'autoload'=>true);
		    ...
		    $wrapped_data['cellStyles'] = $cellStyles ? $stylesList_arr : array('','text-align:left','text-align:left','text-align:left');
	...
	function data_form($mode='add',$data = null){
		...
		if($mode != 'add'){
			...
			$row=$this->spo_options_model->get_data(array('search'=>array(array('search_field' => 'option_id', 'search_text' => $data,'query_method'=>'equal')),'limit' => $limit,'offset'=>$offset)) ;
		...
		//$postData = $this->input->post();
			...
			$key2save = array('option_id' =>$this->input->post('option_id'));
			}
		}
		...
		switch ($mode){
			...
			case "add": //-----------------------------------------------------
				...
				if ($this->input->post('option_name')) {
					if ($this->spo_options_model->exists(array('option_name' => $this->input->post('option_name')))) {
						$message['msgText'] ='Sorry this record already exist!';
						$message['msgType'] ='failure';
						$message['focusField'] ='option_name';
					
					}else{
						$msg = $this->spo_options_model->save($data2save,$key2save);
						if ($msg) {
			...
			case "edit":
				...
				if ($this->input->post('option_id')) {
					...
					if (!$this->spo_options_model->exists(array('option_id' => $this->input->post('option_id')))) {
					 ...
					 $message['focusField'] ='option_name';
						...
						//checking existance
						if ($this->spo_options_model->exists(array('option_name' => $this->input->post('option_name'),'option_id !=' => $this->input->post('option_id')))){
							$message['msgText'] ='Sorry this record already exist!';
							$message['msgType'] ='failure';
							$message['focusField'] ='option_name';
		...
		$wrapper['mode'] = $mode;
		$this->load->view('spo_options_form',$wrapper);
	}
  	...
  	function save($action,$callback = null){
  		...
  		$save_data=array();
  		$key=array();
  		$metadata = $this->spo_options_model->getMetadata();
  		...
  		if($this->AIR_call_flag){
  			...
  			*/  			  			
  			if($this->AR_Utilities->request_method == 'get'){
  				...
  				$added=$this->spo_options_model->save($save_data,$key);  				
    ...
    function key_validate($action,$key) {
	    ...
	    $key_exist=$this->spo_options_model->exists($key);
	...
	function delete($String)
	{
		...
		return($this->spo_options_model->delete($delete));
	}
  	...
  	function loadForm($editable=null,$file='spo_options_form') {
 	...
 	function loadWB($file='spo_options_wb'){
	    ...
	    /**
 		 ...
 		 *
 		 *    http://.../index.php/Spo_options/createDropdown/?sbv=null&sbt=null&format=html&multiple=false&sort=asc&filterby=status_equal=1
 		 *
 		 *
 		 *    1. For normal listing:
 		 *
 		 *    http://.../index.php/Spo_options/createDropdown
 		 *
 		 *    2. For listing with multiple selection
 		 *
 		 *    http://.../index.php/Spo_options/createDropdown/?multiple=true
 		 *
 		 *    3. For listing with pre selection (select by value) for single selection drop down
 		 *
 		 *    http://.../index.php/Spo_options/createDropdown/?sbv=123
 		 *
 		 *    4. For listing with pre selection for multiple (select by value)
 		 *
 		 *    http://.../index.php/Spo_options/createDropdown/?sbv=123,321,345&multiple=true
 		 *
 		 *    5. For listing with pre selection of select by text for single selection dropdown
 		 *
 		 *     http://.../index.php/Spo_options/createDropdown/?sbt=test string&multiple=true
 		 *
 		 *    6. For xml(DHTMLX compatible) dropdown
 		 *
 		 *    http://.../index.php/Spo_options/createDropdown/?format=xml
 		 *
 		 *    7. groupBy
 		 *    http://...../Spo_options/createDropdown/?groupby=users.user_type
 		 *
 		 *	 8. groupCode
 		 *	  http://..../Spo_options/createDropdown/?groupby=users.user_type&groupcodes=1=Staff,2=Admin,4=Manager
 		 *
 		 ...
 		 *
 		 *    groupBy - column to group, should be specified with the table name ex: spo_options.staff_id
 		 *
	...
	public function createDropdown($data = false){
	  	...
	  	$isMultiple = null;
	  	$sort = null;
	    $sort_array = array('spo_options.option_id' => 'ASC');
	  	$filterBy = null;
	   	$searchobj_array = null;
	   	$optionsOnly = false;
	   	$label = null;
	   	$displayClm = 'spo_options.option_name';
	   	$groupBy = null;   
	   	$groupcodes = null;
	   	$nameTag = 'option_id'; 
	  	...
	  	if(is_array($data)){
	  		...
	  		if(count($_POST)>0 || count($_GET)>0){
	        	...
	        	$myParams = count($_POST)>0? $_POST:$_GET;
	  			...
	  			$sort = isset($myParams['sort'])?$myParams['sort']:'spo_options.option_id=ASC';
	  			...
	  			$nameTag = (!empty($nameTag))?$nameTag:'option_id';                       
	      ...
	      $selection = 'spo_options.option_id,spo_options.option_name,spo_options.option_value,spo_options.autoload';
	    
	      // Fetch data from model
	  	  $dataset = $this->spo_options_model->get_data(array(
