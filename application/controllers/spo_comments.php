<?php
		/**
		 * This controller facilitates the access of table spo_comments
		 *
		 * @package    HOF\controller\spo_comments
		 * @version    1.0
		 ...
		 * @modification 
		 */
class Spo_comments extends CI_Controller
{
    ...
    function __construct()
	{
            ...
            $this->load->model('spo_comments_model');
		...
		$this->log_create->init();
            ...
            if($search==true){ 
                ...
                foreach((count($_POST)>0?$_POST:$_GET) as $key => $val){ 
                   ...
                   $data['dataset'] = $this->spo_comments_model->get_data(array('search'=>$searchobj_array,'limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_comments.comment_author_email'=>'Asc')));
            }else {
                   $data['dataset'] = $this->spo_comments_model->get_data(array('limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_comments.comment_author_email'=>'Asc')));
            }   
            ...
            $wrapped_data['collist'] = $collist ? explode(':',$collist) : array('comment_ID','comment_post_ID','comment_author','comment_author_email','comment_author_url','comment_author_IP','comment_date','comment_date_gmt','comment_content','comment_karma','comment_approved','comment_agent','comment_type','comment_parent','user_id')  ;
            ...
            $wrapped_data['widths'] = $widths ? $widths_arr : array('comment_ID'=>75,'comment_post_ID'=>75,'comment_author'=>75,'comment_author_email'=>75,'comment_author_url'=>75,'comment_author_IP'=>75,'comment_date'=>75,'comment_date_gmt'=>75,'comment_content'=>75,'comment_karma'=>75,'comment_approved'=>75,'comment_agent'=>75,'comment_type'=>75,'comment_parent'=>75,'user_id'=>75) ;
		    ...
		    $wrapped_data['visibilityList'] = $visibilityList ? $visibilityList_arr : array('comment_ID'=>true,'comment_post_ID'=>true,'comment_author'=>true,'comment_author_email'=>true,'comment_author_url'=>true,'comment_author_IP'=>true,'comment_date'=>true,'comment_date_gmt'=>true,'comment_content'=>true,'comment_karma'=>true,'comment_approved'=>true,'comment_agent'=>true,'comment_type'=>true,'comment_parent'=>true,'user_id'=>true);
		    ...
		    $wrapped_data['cellStyles'] = $cellStyles ? $stylesList_arr : array('','','','text-align:left','text-align:left','text-align:left','text-align:center','text-align:center','text-align:left','text-align:center','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left');
	...
	function data_form($mode='add',$data = null){
		...
		if($mode != 'add'){
			...
			$row=$this->spo_comments_model->get_data(array('search'=>array(array('search_field' => 'comment_ID', 'search_text' => $data,'query_method'=>'equal')),'limit' => $limit,'offset'=>$offset)) ;
		...
		//$postData = $this->input->post();
			...
			$key2save = array('comment_ID' =>$this->input->post('comment_ID'));
			}
		}
		...
		switch ($mode){
			...
			case "add": //-----------------------------------------------------
				...
				//Check duplicates
				if ($this->input->post('comment_author_email')) {
					if ($this->spo_comments_model->exists(array('comment_author_email' => $this->input->post('comment_author_email')))) {
						$message['msgText'] ='Sorry this record already exist!';
						$message['msgType'] ='failure';
						$message['focusField'] ='comment_author_email';
					
					}else{
						$msg = $this->spo_comments_model->save($data2save,$key2save);
						if ($msg) {
			...
			case "edit":
				...
				if ($this->input->post('comment_ID')) {
					...
					if (!$this->spo_comments_model->exists(array('comment_ID' => $this->input->post('comment_ID')))) {
					 ...
					 $message['focusField'] ='comment_author_email';
						...
						//checking existance
						if ($this->spo_comments_model->exists(array('comment_author_email' => $this->input->post('comment_author_email'),'comment_ID !=' => $this->input->post('comment_ID')))){
							$message['msgText'] ='Sorry this record already exist!';
							$message['msgType'] ='failure';
							$message['focusField'] ='comment_author_email';
		...
		$wrapper['mode'] = $mode;
		$this->load->view('spo_comments_form',$wrapper);
	}
  	...
  	function save($action,$callback = null){
  		...
  		$save_data=array();
  		$key=array();
  		$metadata = $this->spo_comments_model->getMetadata();
  		...
  		if($this->AIR_call_flag){
  			...
  			*/  			  			
  			if($this->AR_Utilities->request_method == 'get'){
  				...
  				$added=$this->spo_comments_model->save($save_data,$key);  				
    ...
    function key_validate($action,$key) {
	    ...
	    $key_exist=$this->spo_comments_model->exists($key);
	...
	function delete($String)
	{
		...
		return($this->spo_comments_model->delete($delete));
	}
  	...
  	function loadForm($editable=null,$file='spo_comments_form') {
 	...
 	function loadWB($file='spo_comments_wb'){
	    ...
	    /**
 		 ...
 		 *
 		 *    http://.../index.php/Spo_comments/createDropdown/?sbv=null&sbt=null&format=html&multiple=false&sort=asc&filterby=status_equal=1
 		 *
 		 *
 		 *    1. For normal listing:
 		 *
 		 *    http://.../index.php/Spo_comments/createDropdown
 		 *
 		 *    2. For listing with multiple selection
 		 *
 		 *    http://.../index.php/Spo_comments/createDropdown/?multiple=true
 		 *
 		 *    3. For listing with pre selection (select by value) for single selection drop down
 		 *
 		 *    http://.../index.php/Spo_comments/createDropdown/?sbv=123
 		 *
 		 *    4. For listing with pre selection for multiple (select by value)
 		 *
 		 *    http://.../index.php/Spo_comments/createDropdown/?sbv=123,321,345&multiple=true
 		 *
 		 *    5. For listing with pre selection of select by text for single selection dropdown
 		 *
 		 *     http://.../index.php/Spo_comments/createDropdown/?sbt=test string&multiple=true
 		 *
 		 *    6. For xml(DHTMLX compatible) dropdown
 		 *
 		 *    http://.../index.php/Spo_comments/createDropdown/?format=xml
 		 *
 		 *    7. groupBy
 		 *    http://...../Spo_comments/createDropdown/?groupby=users.user_type
 		 *
 		 *	 8. groupCode
 		 *	  http://..../Spo_comments/createDropdown/?groupby=users.user_type&groupcodes=1=Staff,2=Admin,4=Manager
 		 *
 		 ...
 		 *
 		 *    groupBy - column to group, should be specified with the table name ex: spo_comments.staff_id
 		 *
	...
	public function createDropdown($data = false){
	  	...
	  	$isMultiple = null;
	  	$sort = null;
	    $sort_array = array('spo_comments.comment_ID' => 'ASC');
	  	$filterBy = null;
	   	$searchobj_array = null;
	   	$optionsOnly = false;
	   	$label = null;
	   	$displayClm = 'spo_comments.comment_author_email';
	   	$groupBy = null;   
	   	$groupcodes = null;
	   	$nameTag = 'comment_ID'; 
	  	...
	  	if(is_array($data)){
	  		...
	  		if(count($_POST)>0 || count($_GET)>0){
	        	...
	        	$myParams = count($_POST)>0? $_POST:$_GET;
	  			...
	  			$sort = isset($myParams['sort'])?$myParams['sort']:'spo_comments.comment_ID=ASC';
	  			...
	  			$nameTag = (!empty($nameTag))?$nameTag:'comment_ID';                       
	      ...
	      $selection = 'spo_comments.comment_ID,spo_comments.comment_post_ID,spo_comments.comment_author,spo_comments.comment_author_email,spo_comments.comment_author_url,spo_comments.comment_author_IP,spo_comments.comment_date,spo_comments.comment_date_gmt,spo_comments.comment_content,spo_comments.comment_karma,spo_comments.comment_approved,spo_comments.comment_agent,spo_comments.comment_type,spo_comments.comment_parent,spo_comments.user_id';
	    
	      // Fetch data from model
	  	  $dataset = $this->spo_comments_model->get_data(array(
