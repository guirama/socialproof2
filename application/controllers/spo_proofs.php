<?php
		/**
		 * This controller facilitates the access of table spo_proofs
		 *
		 * @package    HOF\controller\spo_proofs
		 * @version    1.0
		 ...
		 * @modification 
		 */
class Spo_proofs extends CI_Controller
{
    ...
    function __construct()
	{
            ...
            $this->load->model('spo_proofs_model');
		...
		$this->log_create->init();
            ...
            if($search==true){ 
                ...
                foreach((count($_POST)>0?$_POST:$_GET) as $key => $val){ 
                   ...
                   $data['dataset'] = $this->spo_proofs_model->get_data(array('search'=>$searchobj_array,'limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_proofs.image'=>'Asc')));
            }else {
                   $data['dataset'] = $this->spo_proofs_model->get_data(array('limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_proofs.image'=>'Asc')));
            }   
            ...
            $wrapped_data['collist'] = $collist ? explode(':',$collist) : array('proof_id','client_id','image','comment','status')  ;
            ...
            $wrapped_data['widths'] = $widths ? $widths_arr : array('proof_id'=>75,'client_id'=>75,'image'=>75,'comment'=>75,'status'=>75) ;
		    ...
		    $wrapped_data['visibilityList'] = $visibilityList ? $visibilityList_arr : array('proof_id'=>true,'client_id'=>true,'image'=>true,'comment'=>true,'status'=>true);
		    ...
		    $wrapped_data['cellStyles'] = $cellStyles ? $stylesList_arr : array('text-align:center','text-align:center','text-align:left','text-align:left','text-align:left');
	...
	function data_form($mode='add',$data = null){
		...
		if($mode != 'add'){
			...
			$row=$this->spo_proofs_model->get_data(array('search'=>array(array('search_field' => 'proof_id', 'search_text' => $data,'query_method'=>'equal')),'limit' => $limit,'offset'=>$offset)) ;
		...
		//$postData = $this->input->post();
			...
			if($this->input->post('proof_id') !=''){
			$key2save = array('proof_id' =>$this->input->post('proof_id'));
			}
		}
		...
		switch ($mode){
			...
			case "add": //-----------------------------------------------------
				...
				if ($this->input->post('image')) {
					if ($this->spo_proofs_model->exists(array('image' => $this->input->post('image')))) {
						$message['msgText'] ='Sorry this record already exist!';
						$message['msgType'] ='failure';
						$message['focusField'] ='image';
					
					}else{
						$msg = $this->spo_proofs_model->save($data2save,$key2save);
						if ($msg) {
			...
			case "edit":
				...
				if ($this->input->post('proof_id')) {
					...
					if (!$this->spo_proofs_model->exists(array('proof_id' => $this->input->post('proof_id')))) {
					 ...
					 $message['focusField'] ='image';
						...
						//checking existance
						if ($this->spo_proofs_model->exists(array('image' => $this->input->post('image'),'proof_id !=' => $this->input->post('proof_id')))){
							$message['msgText'] ='Sorry this record already exist!';
							$message['msgType'] ='failure';
							$message['focusField'] ='image';
		...
		$wrapper['mode'] = $mode;
		$this->load->view('spo_proofs_form',$wrapper);
	}
  	...
  	function save($action,$callback = null){
  		...
  		$save_data=array();
  		$key=array();
  		$metadata = $this->spo_proofs_model->getMetadata();
  		...
  		if($this->AIR_call_flag){
  			...
  			*/  			  			
  			if($this->AR_Utilities->request_method == 'get'){
  				...
  				$added=$this->spo_proofs_model->save($save_data,$key);  				
    ...
    function key_validate($action,$key) {
	    ...
	    $key_exist=$this->spo_proofs_model->exists($key);
	...
	function delete($String)
	{
		...
		return($this->spo_proofs_model->delete($delete));
	}
  	...
  	function loadForm($editable=null,$file='spo_proofs_form') {
 	...
 	function loadWB($file='spo_proofs_wb'){
	    ...
	    /**
 		 ...
 		 *
 		 *    http://.../index.php/Spo_proofs/createDropdown/?sbv=null&sbt=null&format=html&multiple=false&sort=asc&filterby=status_equal=1
 		 *
 		 *
 		 *    1. For normal listing:
 		 *
 		 *    http://.../index.php/Spo_proofs/createDropdown
 		 *
 		 *    2. For listing with multiple selection
 		 *
 		 *    http://.../index.php/Spo_proofs/createDropdown/?multiple=true
 		 *
 		 *    3. For listing with pre selection (select by value) for single selection drop down
 		 *
 		 *    http://.../index.php/Spo_proofs/createDropdown/?sbv=123
 		 *
 		 *    4. For listing with pre selection for multiple (select by value)
 		 *
 		 *    http://.../index.php/Spo_proofs/createDropdown/?sbv=123,321,345&multiple=true
 		 *
 		 *    5. For listing with pre selection of select by text for single selection dropdown
 		 *
 		 *     http://.../index.php/Spo_proofs/createDropdown/?sbt=test string&multiple=true
 		 *
 		 *    6. For xml(DHTMLX compatible) dropdown
 		 *
 		 *    http://.../index.php/Spo_proofs/createDropdown/?format=xml
 		 *
 		 *    7. groupBy
 		 *    http://...../Spo_proofs/createDropdown/?groupby=users.user_type
 		 *
 		 *	 8. groupCode
 		 *	  http://..../Spo_proofs/createDropdown/?groupby=users.user_type&groupcodes=1=Staff,2=Admin,4=Manager
 		 *
 		 ...
 		 *
 		 *    groupBy - column to group, should be specified with the table name ex: spo_proofs.staff_id
 		 *
	...
	public function createDropdown($data = false){
	  	...
	  	$isMultiple = null;
	  	$sort = null;
	    $sort_array = array('spo_proofs.proof_id' => 'ASC');
	  	$filterBy = null;
	   	$searchobj_array = null;
	   	$optionsOnly = false;
	   	$label = null;
	   	$displayClm = 'spo_proofs.image';
	   	$groupBy = null;   
	   	$groupcodes = null;
	   	$nameTag = 'proof_id'; 
	  	...
	  	if(is_array($data)){
	  		...
	  		if(count($_POST)>0 || count($_GET)>0){
	        	...
	        	$myParams = count($_POST)>0? $_POST:$_GET;
	  			...
	  			$sort = isset($myParams['sort'])?$myParams['sort']:'spo_proofs.proof_id=ASC';
	  			...
	  			$nameTag = (!empty($nameTag))?$nameTag:'proof_id';                       
	      ...
	      $selection = 'spo_proofs.proof_id,spo_proofs.client_id,spo_proofs.image,spo_proofs.comment,spo_proofs.status';
	    
	      // Fetch data from model
	  	  $dataset = $this->spo_proofs_model->get_data(array(
