<?php
		/**
		 * This controller facilitates the access of table spo_terms
		 *
		 * @package    HOF\controller\spo_terms
		 * @version    1.0
		 ...
		 * @modification 
		 */
class Spo_terms extends CI_Controller
{
    ...
    function __construct()
	{
            ...
            $this->load->model('spo_terms_model');
		...
		$this->log_create->init();
            ...
            if($search==true){ 
                ...
                foreach((count($_POST)>0?$_POST:$_GET) as $key => $val){ 
                   ...
                   $data['dataset'] = $this->spo_terms_model->get_data(array('search'=>$searchobj_array,'limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_terms.name'=>'Asc')));
            }else {
                   $data['dataset'] = $this->spo_terms_model->get_data(array('limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_terms.name'=>'Asc')));
            }   
            ...
            $wrapped_data['collist'] = $collist ? explode(':',$collist) : array('term_id','name','slug','term_group')  ;
            ...
            $wrapped_data['widths'] = $widths ? $widths_arr : array('term_id'=>75,'name'=>75,'slug'=>75,'term_group'=>75) ;
		    ...
		    $wrapped_data['visibilityList'] = $visibilityList ? $visibilityList_arr : array('term_id'=>true,'name'=>true,'slug'=>true,'term_group'=>true);
	...
	function data_form($mode='add',$data = null){
		...
		if($mode != 'add'){
			...
			$row=$this->spo_terms_model->get_data(array('search'=>array(array('search_field' => 'term_id', 'search_text' => $data,'query_method'=>'equal')),'limit' => $limit,'offset'=>$offset)) ;
		...
		//$postData = $this->input->post();
			...
			$key2save = array('term_id' =>$this->input->post('term_id'));
			}
		}
		...
		switch ($mode){
			...
			case "add": //-----------------------------------------------------
				...
				if ($this->input->post('name')) {
					if ($this->spo_terms_model->exists(array('name' => $this->input->post('name')))) {
						$message['msgText'] ='Sorry this record already exist!';
						$message['msgType'] ='failure';
						$message['focusField'] ='name';
					
					}else{
						$msg = $this->spo_terms_model->save($data2save,$key2save);
						if ($msg) {
			...
			case "edit":
				...
				if ($this->input->post('term_id')) {
					...
					if (!$this->spo_terms_model->exists(array('term_id' => $this->input->post('term_id')))) {
					 ...
					 $message['focusField'] ='name';
						...
						//checking existance
						if ($this->spo_terms_model->exists(array('name' => $this->input->post('name'),'term_id !=' => $this->input->post('term_id')))){
							$message['msgText'] ='Sorry this record already exist!';
							$message['msgType'] ='failure';
							$message['focusField'] ='name';
		...
		$wrapper['mode'] = $mode;
		$this->load->view('spo_terms_form',$wrapper);
	}
  	...
  	function save($action,$callback = null){
  		...
  		$save_data=array();
  		$key=array();
  		$metadata = $this->spo_terms_model->getMetadata();
  		...
  		if($this->AIR_call_flag){
  			...
  			*/  			  			
  			if($this->AR_Utilities->request_method == 'get'){
  				...
  				$added=$this->spo_terms_model->save($save_data,$key);  				
    ...
    function key_validate($action,$key) {
	    ...
	    $key_exist=$this->spo_terms_model->exists($key);
	...
	function delete($String)
	{
		...
		return($this->spo_terms_model->delete($delete));
	}
  	...
  	function loadForm($editable=null,$file='spo_terms_form') {
 	...
 	function loadWB($file='spo_terms_wb'){
	    ...
	    /**
 		 ...
 		 *
 		 *    http://.../index.php/Spo_terms/createDropdown/?sbv=null&sbt=null&format=html&multiple=false&sort=asc&filterby=status_equal=1
 		 *
 		 *
 		 *    1. For normal listing:
 		 *
 		 *    http://.../index.php/Spo_terms/createDropdown
 		 *
 		 *    2. For listing with multiple selection
 		 *
 		 *    http://.../index.php/Spo_terms/createDropdown/?multiple=true
 		 *
 		 *    3. For listing with pre selection (select by value) for single selection drop down
 		 *
 		 *    http://.../index.php/Spo_terms/createDropdown/?sbv=123
 		 *
 		 *    4. For listing with pre selection for multiple (select by value)
 		 *
 		 *    http://.../index.php/Spo_terms/createDropdown/?sbv=123,321,345&multiple=true
 		 *
 		 *    5. For listing with pre selection of select by text for single selection dropdown
 		 *
 		 *     http://.../index.php/Spo_terms/createDropdown/?sbt=test string&multiple=true
 		 *
 		 *    6. For xml(DHTMLX compatible) dropdown
 		 *
 		 *    http://.../index.php/Spo_terms/createDropdown/?format=xml
 		 *
 		 *    7. groupBy
 		 *    http://...../Spo_terms/createDropdown/?groupby=users.user_type
 		 *
 		 *	 8. groupCode
 		 *	  http://..../Spo_terms/createDropdown/?groupby=users.user_type&groupcodes=1=Staff,2=Admin,4=Manager
 		 *
 		 ...
 		 *
 		 *    groupBy - column to group, should be specified with the table name ex: spo_terms.staff_id
 		 *
	...
	public function createDropdown($data = false){
	  	...
	  	$isMultiple = null;
	  	$sort = null;
	    $sort_array = array('spo_terms.term_id' => 'ASC');
	  	$filterBy = null;
	   	$searchobj_array = null;
	   	$optionsOnly = false;
	   	$label = null;
	   	$displayClm = 'spo_terms.name';
	   	$groupBy = null;   
	   	$groupcodes = null;
	   	$nameTag = 'term_id'; 
	  	...
	  	if(is_array($data)){
	  		...
	  		if(count($_POST)>0 || count($_GET)>0){
	        	...
	        	$myParams = count($_POST)>0? $_POST:$_GET;
	  			...
	  			$sort = isset($myParams['sort'])?$myParams['sort']:'spo_terms.term_id=ASC';
	  			...
	  			$nameTag = (!empty($nameTag))?$nameTag:'term_id';                       
	      ...
	      $selection = 'spo_terms.term_id,spo_terms.name,spo_terms.slug,spo_terms.term_group';
	    
	      // Fetch data from model
	  	  $dataset = $this->spo_terms_model->get_data(array(
