<?php
		/**
		 * This controller facilitates the access of table spo_clients
		 *
		 * @package    HOF\controller\spo_clients
		 * @version    1.0
		 ...
		 * @modification 
		 */
class Spo_clients extends CI_Controller
{
    ...
    function __construct()
	{
            ...
            $this->load->model('spo_clients_model');
		...
		$this->log_create->init();
            ...
            if($search==true){ 
                ...
                foreach((count($_POST)>0?$_POST:$_GET) as $key => $val){ 
                   ...
                   $data['dataset'] = $this->spo_clients_model->get_data(array('search'=>$searchobj_array,'limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_clients.client_name'=>'Asc')));
            }else {
                   $data['dataset'] = $this->spo_clients_model->get_data(array('limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_clients.client_name'=>'Asc')));
            }   
            ...
            $wrapped_data['collist'] = $collist ? explode(':',$collist) : array('client_id','client_name','username','password','practice_name','website','address','email','phone','gmr_id','app_form_id','call_tracker','dr_img','dr_logo')  ;
            ...
            $wrapped_data['widths'] = $widths ? $widths_arr : array('client_id'=>75,'client_name'=>75,'username'=>75,'password'=>75,'practice_name'=>75,'website'=>75,'address'=>75,'email'=>75,'phone'=>75,'gmr_id'=>75,'app_form_id'=>75,'call_tracker'=>75,'dr_img'=>75,'dr_logo'=>75) ;
		    ...
		    $wrapped_data['visibilityList'] = $visibilityList ? $visibilityList_arr : array('client_id'=>true,'client_name'=>true,'username'=>true,'password'=>true,'practice_name'=>true,'website'=>true,'address'=>true,'email'=>true,'phone'=>true,'gmr_id'=>true,'app_form_id'=>true,'call_tracker'=>true,'dr_img'=>true,'dr_logo'=>true);
		    ...
		    $wrapped_data['cellStyles'] = $cellStyles ? $stylesList_arr : array('text-align:center','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:center','text-align:left','text-align:left','text-align:left','text-align:left');
	...
	function data_form($mode='add',$data = null){
		...
		if($mode != 'add'){
			...
			$row=$this->spo_clients_model->get_data(array('search'=>array(array('search_field' => 'client_id', 'search_text' => $data,'query_method'=>'equal')),'limit' => $limit,'offset'=>$offset)) ;
		...
		//$postData = $this->input->post();
			...
			$key2save = array('client_id' =>$this->input->post('client_id'));
			}
		}
		...
		switch ($mode){
			...
			case "add": //-----------------------------------------------------
				...
				if ($this->input->post('client_name')) {
					if ($this->spo_clients_model->exists(array('client_name' => $this->input->post('client_name')))) {
						$message['msgText'] ='Sorry this record already exist!';
						$message['msgType'] ='failure';
						$message['focusField'] ='client_name';
					
					}else{
						$msg = $this->spo_clients_model->save($data2save,$key2save);
						if ($msg) {
			...
			case "edit":
				...
				if ($this->input->post('client_id')) {
					...
					if (!$this->spo_clients_model->exists(array('client_id' => $this->input->post('client_id')))) {
					 ...
					 $message['focusField'] ='client_name';
						...
						//checking existance
						if ($this->spo_clients_model->exists(array('client_name' => $this->input->post('client_name'),'client_id !=' => $this->input->post('client_id')))){
							$message['msgText'] ='Sorry this record already exist!';
							$message['msgType'] ='failure';
							$message['focusField'] ='client_name';
		...
		$wrapper['mode'] = $mode;
		$this->load->view('spo_clients_form',$wrapper);
	}
  	...
  	function save($action,$callback = null){
  		...
  		$save_data=array();
  		$key=array();
  		$metadata = $this->spo_clients_model->getMetadata();
  		...
  		if($this->AIR_call_flag){
  			...
  			*/  			  			
  			if($this->AR_Utilities->request_method == 'get'){
  				...
  				$added=$this->spo_clients_model->save($save_data,$key);  				
    ...
    function key_validate($action,$key) {
	    ...
	    $key_exist=$this->spo_clients_model->exists($key);
	...
	function delete($String)
	{
		...
		return($this->spo_clients_model->delete($delete));
	}
  	...
  	function loadForm($editable=null,$file='spo_clients_form') {
 	...
 	function loadWB($file='spo_clients_wb'){
	    ...
	    /**
 		 ...
 		 *
 		 *    http://.../index.php/Spo_clients/createDropdown/?sbv=null&sbt=null&format=html&multiple=false&sort=asc&filterby=status_equal=1
 		 *
 		 *
 		 *    1. For normal listing:
 		 *
 		 *    http://.../index.php/Spo_clients/createDropdown
 		 *
 		 *    2. For listing with multiple selection
 		 *
 		 *    http://.../index.php/Spo_clients/createDropdown/?multiple=true
 		 *
 		 *    3. For listing with pre selection (select by value) for single selection drop down
 		 *
 		 *    http://.../index.php/Spo_clients/createDropdown/?sbv=123
 		 *
 		 *    4. For listing with pre selection for multiple (select by value)
 		 *
 		 *    http://.../index.php/Spo_clients/createDropdown/?sbv=123,321,345&multiple=true
 		 *
 		 *    5. For listing with pre selection of select by text for single selection dropdown
 		 *
 		 *     http://.../index.php/Spo_clients/createDropdown/?sbt=test string&multiple=true
 		 *
 		 *    6. For xml(DHTMLX compatible) dropdown
 		 *
 		 *    http://.../index.php/Spo_clients/createDropdown/?format=xml
 		 *
 		 *    7. groupBy
 		 *    http://...../Spo_clients/createDropdown/?groupby=users.user_type
 		 *
 		 *	 8. groupCode
 		 *	  http://..../Spo_clients/createDropdown/?groupby=users.user_type&groupcodes=1=Staff,2=Admin,4=Manager
 		 *
 		 ...
 		 *
 		 *    groupBy - column to group, should be specified with the table name ex: spo_clients.staff_id
 		 *
	...
	public function createDropdown($data = false){
	  	...
	  	$isMultiple = null;
	  	$sort = null;
	    $sort_array = array('spo_clients.client_id' => 'ASC');
	  	$filterBy = null;
	   	$searchobj_array = null;
	   	$optionsOnly = false;
	   	$label = null;
	   	$displayClm = 'spo_clients.client_name';
	   	$groupBy = null;   
	   	$groupcodes = null;
	   	$nameTag = 'client_id'; 
	  	...
	  	if(is_array($data)){
	  		...
	  		if(count($_POST)>0 || count($_GET)>0){
	        	...
	        	$myParams = count($_POST)>0? $_POST:$_GET;
	  			...
	  			$sort = isset($myParams['sort'])?$myParams['sort']:'spo_clients.client_id=ASC';
	  			...
	  			$nameTag = (!empty($nameTag))?$nameTag:'client_id';                       
	      ...
	      $selection = 'spo_clients.client_id,spo_clients.client_name,spo_clients.username,spo_clients.password,spo_clients.practice_name,spo_clients.website,spo_clients.address,spo_clients.email,spo_clients.phone,spo_clients.gmr_id,spo_clients.app_form_id,spo_clients.call_tracker,spo_clients.dr_img,spo_clients.dr_logo';
	    
	      // Fetch data from model
	  	  $dataset = $this->spo_clients_model->get_data(array(
