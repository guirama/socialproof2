<?php
		/**
		 * This controller facilitates the access of table spo_term_taxonomy
		 *
		 * @package    HOF\controller\spo_term_taxonomy
		 * @version    1.0
		 ...
		 * @modification 
		 */
class Spo_term_taxonomy extends CI_Controller
{
    ...
    function __construct()
	{
            ...
            $this->load->model('spo_term_taxonomy_model');
		...
		$this->log_create->init();
            ...
            if($search==true){ 
                ...
                foreach((count($_POST)>0?$_POST:$_GET) as $key => $val){ 
                   ...
                   $data['dataset'] = $this->spo_term_taxonomy_model->get_data(array('search'=>$searchobj_array,'limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_term_taxonomy.taxonomy'=>'Asc')));
            }else {
                   $data['dataset'] = $this->spo_term_taxonomy_model->get_data(array('limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_term_taxonomy.taxonomy'=>'Asc')));
            }   
            ...
            $wrapped_data['collist'] = $collist ? explode(':',$collist) : array('term_taxonomy_id','term_id','taxonomy','description','parent','count')  ;
            ...
            $wrapped_data['widths'] = $widths ? $widths_arr : array('term_taxonomy_id'=>75,'term_id'=>75,'taxonomy'=>75,'description'=>75,'parent'=>75,'count'=>75) ;
		    ...
		    $wrapped_data['visibilityList'] = $visibilityList ? $visibilityList_arr : array('term_taxonomy_id'=>true,'term_id'=>true,'taxonomy'=>true,'description'=>true,'parent'=>true,'count'=>true);
		    ...
		    $wrapped_data['cellStyles'] = $cellStyles ? $stylesList_arr : array('','','text-align:left','text-align:left','text-align:left','text-align:left');
	...
	function data_form($mode='add',$data = null){
		...
		if($mode != 'add'){
			...
			$row=$this->spo_term_taxonomy_model->get_data(array('search'=>array(array('search_field' => 'term_taxonomy_id', 'search_text' => $data,'query_method'=>'equal')),'limit' => $limit,'offset'=>$offset)) ;
		...
		//$postData = $this->input->post();
			...
			if($this->input->post('term_taxonomy_id') !=''){
			$key2save = array('term_taxonomy_id' =>$this->input->post('term_taxonomy_id'));
			}
		}
		...
		switch ($mode){
			...
			case "add": //-----------------------------------------------------
				...
				//Check duplicates
				if ($this->input->post('taxonomy')) {
					if ($this->spo_term_taxonomy_model->exists(array('taxonomy' => $this->input->post('taxonomy')))) {
						$message['msgText'] ='Sorry this record already exist!';
						$message['msgType'] ='failure';
						$message['focusField'] ='taxonomy';
					
					}else{
						$msg = $this->spo_term_taxonomy_model->save($data2save,$key2save);
						if ($msg) {
			...
			case "edit":
				...
				if ($this->input->post('term_taxonomy_id')) {
					...
					if (!$this->spo_term_taxonomy_model->exists(array('term_taxonomy_id' => $this->input->post('term_taxonomy_id')))) {
					 ...
					 $message['focusField'] ='taxonomy';
						...
						//checking existance
						if ($this->spo_term_taxonomy_model->exists(array('taxonomy' => $this->input->post('taxonomy'),'term_taxonomy_id !=' => $this->input->post('term_taxonomy_id')))){
							$message['msgText'] ='Sorry this record already exist!';
							$message['msgType'] ='failure';
							$message['focusField'] ='taxonomy';
		...
		$wrapper['mode'] = $mode;
		$this->load->view('spo_term_taxonomy_form',$wrapper);
	}
  	...
  	function save($action,$callback = null){
  		...
  		$save_data=array();
  		$key=array();
  		$metadata = $this->spo_term_taxonomy_model->getMetadata();
  		...
  		if($this->AIR_call_flag){
  			...
  			*/  			  			
  			if($this->AR_Utilities->request_method == 'get'){
  				...
  				$added=$this->spo_term_taxonomy_model->save($save_data,$key);  				
    ...
    function key_validate($action,$key) {
	    ...
	    $key_exist=$this->spo_term_taxonomy_model->exists($key);
	...
	function delete($String)
	{
		...
		return($this->spo_term_taxonomy_model->delete($delete));
	}
  	...
  	function loadForm($editable=null,$file='spo_term_taxonomy_form') {
 	...
 	function loadWB($file='spo_term_taxonomy_wb'){
	    ...
	    /**
 		 ...
 		 *
 		 *    http://.../index.php/Spo_term_taxonomy/createDropdown/?sbv=null&sbt=null&format=html&multiple=false&sort=asc&filterby=status_equal=1
 		 *
 		 *
 		 *    1. For normal listing:
 		 *
 		 *    http://.../index.php/Spo_term_taxonomy/createDropdown
 		 *
 		 *    2. For listing with multiple selection
 		 *
 		 *    http://.../index.php/Spo_term_taxonomy/createDropdown/?multiple=true
 		 *
 		 *    3. For listing with pre selection (select by value) for single selection drop down
 		 *
 		 *    http://.../index.php/Spo_term_taxonomy/createDropdown/?sbv=123
 		 *
 		 *    4. For listing with pre selection for multiple (select by value)
 		 *
 		 *    http://.../index.php/Spo_term_taxonomy/createDropdown/?sbv=123,321,345&multiple=true
 		 *
 		 *    5. For listing with pre selection of select by text for single selection dropdown
 		 *
 		 *     http://.../index.php/Spo_term_taxonomy/createDropdown/?sbt=test string&multiple=true
 		 *
 		 *    6. For xml(DHTMLX compatible) dropdown
 		 *
 		 *    http://.../index.php/Spo_term_taxonomy/createDropdown/?format=xml
 		 *
 		 *    7. groupBy
 		 *    http://...../Spo_term_taxonomy/createDropdown/?groupby=users.user_type
 		 *
 		 *	 8. groupCode
 		 *	  http://..../Spo_term_taxonomy/createDropdown/?groupby=users.user_type&groupcodes=1=Staff,2=Admin,4=Manager
 		 *
 		 ...
 		 *
 		 *    groupBy - column to group, should be specified with the table name ex: spo_term_taxonomy.staff_id
 		 *
	...
	public function createDropdown($data = false){
	  	...
	  	$isMultiple = null;
	  	$sort = null;
	    $sort_array = array('spo_term_taxonomy.term_taxonomy_id' => 'ASC');
	  	$filterBy = null;
	   	$searchobj_array = null;
	   	$optionsOnly = false;
	   	$label = null;
	   	$displayClm = 'spo_term_taxonomy.taxonomy';
	   	$groupBy = null;   
	   	$groupcodes = null;
	   	$nameTag = 'term_taxonomy_id'; 
	  	...
	  	if(is_array($data)){
	  		...
	  		if(count($_POST)>0 || count($_GET)>0){
	        	...
	        	$myParams = count($_POST)>0? $_POST:$_GET;
	  			...
	  			$sort = isset($myParams['sort'])?$myParams['sort']:'spo_term_taxonomy.term_taxonomy_id=ASC';
	  			...
	  			$nameTag = (!empty($nameTag))?$nameTag:'term_taxonomy_id';                       
	      ...
	      $selection = 'spo_term_taxonomy.term_taxonomy_id,spo_term_taxonomy.term_id,spo_term_taxonomy.taxonomy,spo_term_taxonomy.description,spo_term_taxonomy.parent,spo_term_taxonomy.count';
	    
	      // Fetch data from model
	  	  $dataset = $this->spo_term_taxonomy_model->get_data(array(
