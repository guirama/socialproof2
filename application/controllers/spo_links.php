<?php
		/**
		 * This controller facilitates the access of table spo_links
		 *
		 * @package    HOF\controller\spo_links
		 * @version    1.0
		 ...
		 * @modification 
		 */
class Spo_links extends CI_Controller
{
    ...
    function __construct()
	{
            ...
            $this->load->model('spo_links_model');
		...
		$this->log_create->init();
            ...
            if($search==true){ 
                ...
                foreach((count($_POST)>0?$_POST:$_GET) as $key => $val){ 
                   ...
                   $data['dataset'] = $this->spo_links_model->get_data(array('search'=>$searchobj_array,'limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_links.link_url'=>'Asc')));
            }else {
                   $data['dataset'] = $this->spo_links_model->get_data(array('limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_links.link_url'=>'Asc')));
            }   
            ...
            $wrapped_data['collist'] = $collist ? explode(':',$collist) : array('link_id','link_url','link_name','link_image','link_target','link_description','link_visible','link_owner','link_rating','link_updated','link_rel','link_notes','link_rss')  ;
            ...
            $wrapped_data['widths'] = $widths ? $widths_arr : array('link_id'=>75,'link_url'=>75,'link_name'=>75,'link_image'=>75,'link_target'=>75,'link_description'=>75,'link_visible'=>75,'link_owner'=>75,'link_rating'=>75,'link_updated'=>75,'link_rel'=>75,'link_notes'=>75,'link_rss'=>75) ;
		    ...
		    $wrapped_data['visibilityList'] = $visibilityList ? $visibilityList_arr : array('link_id'=>true,'link_url'=>true,'link_name'=>true,'link_image'=>true,'link_target'=>true,'link_description'=>true,'link_visible'=>true,'link_owner'=>true,'link_rating'=>true,'link_updated'=>true,'link_rel'=>true,'link_notes'=>true,'link_rss'=>true);
		    ...
		    $wrapped_data['cellStyles'] = $cellStyles ? $stylesList_arr : array('','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:center','text-align:center','text-align:left','text-align:left','text-align:left');
	...
	function data_form($mode='add',$data = null){
		...
		if($mode != 'add'){
			...
			$row=$this->spo_links_model->get_data(array('search'=>array(array('search_field' => 'link_id', 'search_text' => $data,'query_method'=>'equal')),'limit' => $limit,'offset'=>$offset)) ;
		...
		//$postData = $this->input->post();
			...
			$key2save = array('link_id' =>$this->input->post('link_id'));
			}
		}
		...
		switch ($mode){
			...
			case "add": //-----------------------------------------------------
				...
				if ($this->input->post('link_url')) {
					if ($this->spo_links_model->exists(array('link_url' => $this->input->post('link_url')))) {
						$message['msgText'] ='Sorry this record already exist!';
						$message['msgType'] ='failure';
						$message['focusField'] ='link_url';
					
					}else{
						$msg = $this->spo_links_model->save($data2save,$key2save);
						if ($msg) {
			...
			case "edit":
				...
				if ($this->input->post('link_id')) {
					...
					if (!$this->spo_links_model->exists(array('link_id' => $this->input->post('link_id')))) {
					 ...
					 $message['focusField'] ='link_url';
						...
						//checking existance
						if ($this->spo_links_model->exists(array('link_url' => $this->input->post('link_url'),'link_id !=' => $this->input->post('link_id')))){
							$message['msgText'] ='Sorry this record already exist!';
							$message['msgType'] ='failure';
							$message['focusField'] ='link_url';
		...
		$wrapper['mode'] = $mode;
		$this->load->view('spo_links_form',$wrapper);
	}
  	...
  	function save($action,$callback = null){
  		...
  		$save_data=array();
  		$key=array();
  		$metadata = $this->spo_links_model->getMetadata();
  		...
  		if($this->AIR_call_flag){
  			...
  			*/  			  			
  			if($this->AR_Utilities->request_method == 'get'){
  				...
  				$added=$this->spo_links_model->save($save_data,$key);  				
    ...
    function key_validate($action,$key) {
	    ...
	    $key_exist=$this->spo_links_model->exists($key);
	...
	function delete($String)
	{
		...
		return($this->spo_links_model->delete($delete));
	}
  	...
  	function loadForm($editable=null,$file='spo_links_form') {
 	...
 	function loadWB($file='spo_links_wb'){
	    ...
	    /**
 		 ...
 		 *
 		 *    http://.../index.php/Spo_links/createDropdown/?sbv=null&sbt=null&format=html&multiple=false&sort=asc&filterby=status_equal=1
 		 *
 		 *
 		 *    1. For normal listing:
 		 *
 		 *    http://.../index.php/Spo_links/createDropdown
 		 *
 		 *    2. For listing with multiple selection
 		 *
 		 *    http://.../index.php/Spo_links/createDropdown/?multiple=true
 		 *
 		 *    3. For listing with pre selection (select by value) for single selection drop down
 		 *
 		 *    http://.../index.php/Spo_links/createDropdown/?sbv=123
 		 *
 		 *    4. For listing with pre selection for multiple (select by value)
 		 *
 		 *    http://.../index.php/Spo_links/createDropdown/?sbv=123,321,345&multiple=true
 		 *
 		 *    5. For listing with pre selection of select by text for single selection dropdown
 		 *
 		 *     http://.../index.php/Spo_links/createDropdown/?sbt=test string&multiple=true
 		 *
 		 *    6. For xml(DHTMLX compatible) dropdown
 		 *
 		 *    http://.../index.php/Spo_links/createDropdown/?format=xml
 		 *
 		 *    7. groupBy
 		 *    http://...../Spo_links/createDropdown/?groupby=users.user_type
 		 *
 		 *	 8. groupCode
 		 *	  http://..../Spo_links/createDropdown/?groupby=users.user_type&groupcodes=1=Staff,2=Admin,4=Manager
 		 *
 		 ...
 		 *
 		 *    groupBy - column to group, should be specified with the table name ex: spo_links.staff_id
 		 *
	...
	public function createDropdown($data = false){
	  	...
	  	$isMultiple = null;
	  	$sort = null;
	    $sort_array = array('spo_links.link_id' => 'ASC');
	  	$filterBy = null;
	   	$searchobj_array = null;
	   	$optionsOnly = false;
	   	$label = null;
	   	$displayClm = 'spo_links.link_url';
	   	$groupBy = null;   
	   	$groupcodes = null;
	   	$nameTag = 'link_id'; 
	  	...
	  	if(is_array($data)){
	  		...
	  		if(count($_POST)>0 || count($_GET)>0){
	        	...
	        	$myParams = count($_POST)>0? $_POST:$_GET;
	  			...
	  			$sort = isset($myParams['sort'])?$myParams['sort']:'spo_links.link_id=ASC';
	  			...
	  			$nameTag = (!empty($nameTag))?$nameTag:'link_id';                       
	      ...
	      $selection = 'spo_links.link_id,spo_links.link_url,spo_links.link_name,spo_links.link_image,spo_links.link_target,spo_links.link_description,spo_links.link_visible,spo_links.link_owner,spo_links.link_rating,spo_links.link_updated,spo_links.link_rel,spo_links.link_notes,spo_links.link_rss';
	    
	      // Fetch data from model
	  	  $dataset = $this->spo_links_model->get_data(array(
