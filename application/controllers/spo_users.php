<?php
		/**
		 * This controller facilitates the access of table spo_users
		 *
		 * @package    HOF\controller\spo_users
		 * @version    1.0
		 ...
		 * @modification 
		 */
class Spo_users extends CI_Controller
{
		...
		$this->log_create->init();
            ...
            if($search==true){ 
                ...
                foreach((count($_POST)>0?$_POST:$_GET) as $key => $val){ 
                   ...
                   $data['dataset'] = $this->spo_users_model->get_data(array('search'=>$searchobj_array,'limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_users.user_login'=>'Asc')));
            }else {
                   $data['dataset'] = $this->spo_users_model->get_data(array('limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_users.user_login'=>'Asc')));
            }   
            ...
            $wrapped_data['collist'] = $collist ? explode(':',$collist) : array('ID','user_login','user_pass','user_nicename','user_email','user_url','user_registered','user_activation_key','user_status','display_name')  ;
            ...
            $wrapped_data['widths'] = $widths ? $widths_arr : array('ID'=>75,'user_login'=>75,'user_pass'=>75,'user_nicename'=>75,'user_email'=>75,'user_url'=>75,'user_registered'=>75,'user_activation_key'=>75,'user_status'=>75,'display_name'=>75) ;
		    ...
		    $wrapped_data['visibilityList'] = $visibilityList ? $visibilityList_arr : array('ID'=>true,'user_login'=>true,'user_pass'=>true,'user_nicename'=>true,'user_email'=>true,'user_url'=>true,'user_registered'=>true,'user_activation_key'=>true,'user_status'=>true,'display_name'=>true);
		    ...
		    $wrapped_data['cellStyles'] = $cellStyles ? $stylesList_arr : array('','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:center','text-align:left','text-align:center','text-align:left');
	...
	function data_form($mode='add',$data = null){
		...
		if($mode != 'add'){
			...
			$row=$this->spo_users_model->get_data(array('search'=>array(array('search_field' => 'ID', 'search_text' => $data,'query_method'=>'equal')),'limit' => $limit,'offset'=>$offset)) ;
		...
		switch ($mode){
			...
			case "add": //-----------------------------------------------------
				...
				if ($this->input->post('user_login')) {
					if ($this->spo_users_model->exists(array('user_login' => $this->input->post('user_login')))) {
						$message['msgText'] ='Sorry this record already exist!';
						$message['msgType'] ='failure';
						$message['focusField'] ='user_login';
					
					}else{
						$msg = $this->spo_users_model->save($data2save,$key2save);
						if ($msg) {
			...
			case "edit":
				...
				if ($this->input->post('ID')) {
					...
					if (!$this->spo_users_model->exists(array('ID' => $this->input->post('ID')))) {
					 ...
					 $message['focusField'] ='user_login';
						...
						//checking existance
						if ($this->spo_users_model->exists(array('user_login' => $this->input->post('user_login'),'ID !=' => $this->input->post('ID')))){
							$message['msgText'] ='Sorry this record already exist!';
							$message['msgType'] ='failure';
							$message['focusField'] ='user_login';
		...
		$wrapper['mode'] = $mode;
		$this->load->view('spo_users_form',$wrapper);
	}
  	...
  	function save($action,$callback = null){
  		...
  		$save_data=array();
  		$key=array();
  		$metadata = $this->spo_users_model->getMetadata();
  		...
  		if($this->AIR_call_flag){
  			...
  			*/  			  			
  			if($this->AR_Utilities->request_method == 'get'){
  				...
  				$added=$this->spo_users_model->save($save_data,$key);  				
    ...
    function key_validate($action,$key) {
	    ...
	    $key_exist=$this->spo_users_model->exists($key);
	...
	function delete($String)
	{
		...
		return($this->spo_users_model->delete($delete));
	}
  	...
  	function loadForm($editable=null,$file='spo_users_form') {
 	...
 	function loadWB($file='spo_users_wb'){
	    ...
	    /**
 		 ...
 		 *
 		 *    http://.../index.php/Spo_users/createDropdown/?sbv=null&sbt=null&format=html&multiple=false&sort=asc&filterby=status_equal=1
 		 *
 		 *
 		 *    1. For normal listing:
 		 *
 		 *    http://.../index.php/Spo_users/createDropdown
 		 *
 		 *    2. For listing with multiple selection
 		 *
 		 *    http://.../index.php/Spo_users/createDropdown/?multiple=true
 		 *
 		 *    3. For listing with pre selection (select by value) for single selection drop down
 		 *
 		 *    http://.../index.php/Spo_users/createDropdown/?sbv=123
 		 *
 		 *    4. For listing with pre selection for multiple (select by value)
 		 *
 		 *    http://.../index.php/Spo_users/createDropdown/?sbv=123,321,345&multiple=true
 		 *
 		 *    5. For listing with pre selection of select by text for single selection dropdown
 		 *
 		 *     http://.../index.php/Spo_users/createDropdown/?sbt=test string&multiple=true
 		 *
 		 *    6. For xml(DHTMLX compatible) dropdown
 		 *
 		 *    http://.../index.php/Spo_users/createDropdown/?format=xml
 		 *
 		 *    7. groupBy
 		 *    http://...../Spo_users/createDropdown/?groupby=users.user_type
 		 *
 		 *	 8. groupCode
 		 *	  http://..../Spo_users/createDropdown/?groupby=users.user_type&groupcodes=1=Staff,2=Admin,4=Manager
 		 *
 		 ...
 		 *
 		 *    groupBy - column to group, should be specified with the table name ex: spo_users.staff_id
 		 *
	...
	public function createDropdown($data = false){
	  	...
	  	$isMultiple = null;
	  	$sort = null;
	    $sort_array = array('spo_users.ID' => 'ASC');
	  	$filterBy = null;
	   	$searchobj_array = null;
	   	$optionsOnly = false;
	   	$label = null;
	   	$displayClm = 'spo_users.user_login';
	   	$groupBy = null;   
	   	$groupcodes = null;
	   	$nameTag = 'ID'; 
	  	...
	  	if(is_array($data)){
	  		...
	  		$sort = isset($data['sort'])?$data['sort']:'spo_users.ID=ASC';
	  		...
	  		if(count($_POST)>0 || count($_GET)>0){
	        	...
	        	$myParams = count($_POST)>0? $_POST:$_GET;
	  			...
	  			$sort = isset($myParams['sort'])?$myParams['sort']:'spo_users.ID=ASC';
	      ...
	      $selection = 'spo_users.ID,spo_users.user_login,spo_users.user_pass,spo_users.user_nicename,spo_users.user_email,spo_users.user_url,spo_users.user_registered,spo_users.user_activation_key,spo_users.user_status,spo_users.display_name';
	    
	      // Fetch data from model
	  	  $dataset = $this->spo_users_model->get_data(array(
