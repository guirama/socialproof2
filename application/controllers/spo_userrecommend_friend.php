<?php
		/**
		 * This controller facilitates the access of table spo_userrecommend_friend
		 *
		 * @package    HOF\controller\spo_userrecommend_friend
		 * @version    1.0
		 ...
		 * @modification 
		 */
class Spo_userrecommend_friend extends CI_Controller
{
    ...
    function __construct()
	{
            ...
            $this->load->model('spo_userrecommend_friend_model');
		...
		$this->log_create->init();
            ...
            if($search==true){ 
                ...
                foreach((count($_POST)>0?$_POST:$_GET) as $key => $val){ 
                   ...
                   $data['dataset'] = $this->spo_userrecommend_friend_model->get_data(array('search'=>$searchobj_array,'limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_userrecommend_friend.name'=>'Asc')));
            }else {
                   $data['dataset'] = $this->spo_userrecommend_friend_model->get_data(array('limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_userrecommend_friend.name'=>'Asc')));
            }   
            ...
            $wrapped_data['collist'] = $collist ? explode(':',$collist) : array('recommend_id','user_id','name','friend_name','friend_email','friend_mobile','ip','date')  ;
            ...
            $wrapped_data['widths'] = $widths ? $widths_arr : array('recommend_id'=>75,'user_id'=>75,'name'=>75,'friend_name'=>75,'friend_email'=>75,'friend_mobile'=>75,'ip'=>75,'date'=>75) ;
		    ...
		    $wrapped_data['visibilityList'] = $visibilityList ? $visibilityList_arr : array('recommend_id'=>true,'user_id'=>true,'name'=>true,'friend_name'=>true,'friend_email'=>true,'friend_mobile'=>true,'ip'=>true,'date'=>true);
		    ...
		    $wrapped_data['cellStyles'] = $cellStyles ? $stylesList_arr : array('text-align:center','text-align:center','text-align:left','text-align:left','text-align:left','text-align:left','text-align:left','text-align:center');
	...
	function data_form($mode='add',$data = null){
		...
		if($mode != 'add'){
			...
			$row=$this->spo_userrecommend_friend_model->get_data(array('search'=>array(array('search_field' => 'recommend_id', 'search_text' => $data,'query_method'=>'equal')),'limit' => $limit,'offset'=>$offset)) ;
		...
		//$postData = $this->input->post();
			...
			$key2save = array('recommend_id' =>$this->input->post('recommend_id'));
			}
		}
		...
		switch ($mode){
			...
			case "add": //-----------------------------------------------------
				...
				if ($this->input->post('name')) {
					if ($this->spo_userrecommend_friend_model->exists(array('name' => $this->input->post('name')))) {
						$message['msgText'] ='Sorry this record already exist!';
						$message['msgType'] ='failure';
						$message['focusField'] ='name';
					
					}else{
						$msg = $this->spo_userrecommend_friend_model->save($data2save,$key2save);
						if ($msg) {
			...
			case "edit":
				...
				if ($this->input->post('recommend_id')) {
					...
					if (!$this->spo_userrecommend_friend_model->exists(array('recommend_id' => $this->input->post('recommend_id')))) {
					 ...
					 $message['focusField'] ='name';
						...
						//checking existance
						if ($this->spo_userrecommend_friend_model->exists(array('name' => $this->input->post('name'),'recommend_id !=' => $this->input->post('recommend_id')))){
							$message['msgText'] ='Sorry this record already exist!';
							$message['msgType'] ='failure';
							$message['focusField'] ='name';
		...
		$wrapper['mode'] = $mode;
		$this->load->view('spo_userrecommend_friend_form',$wrapper);
	}
  	...
  	function save($action,$callback = null){
  		...
  		$save_data=array();
  		$key=array();
  		$metadata = $this->spo_userrecommend_friend_model->getMetadata();
  		...
  		if($this->AIR_call_flag){
  			...
  			*/  			  			
  			if($this->AR_Utilities->request_method == 'get'){
  				...
  				$added=$this->spo_userrecommend_friend_model->save($save_data,$key);  				
    ...
    function key_validate($action,$key) {
	    ...
	    $key_exist=$this->spo_userrecommend_friend_model->exists($key);
	...
	function delete($String)
	{
		...
		return($this->spo_userrecommend_friend_model->delete($delete));
	}
  	...
  	function loadForm($editable=null,$file='spo_userrecommend_friend_form') {
 	...
 	function loadWB($file='spo_userrecommend_friend_wb'){
	    ...
	    /**
 		 ...
 		 *
 		 *    http://.../index.php/Spo_userrecommend_friend/createDropdown/?sbv=null&sbt=null&format=html&multiple=false&sort=asc&filterby=status_equal=1
 		 *
 		 *
 		 *    1. For normal listing:
 		 *
 		 *    http://.../index.php/Spo_userrecommend_friend/createDropdown
 		 *
 		 *    2. For listing with multiple selection
 		 *
 		 *    http://.../index.php/Spo_userrecommend_friend/createDropdown/?multiple=true
 		 *
 		 *    3. For listing with pre selection (select by value) for single selection drop down
 		 *
 		 *    http://.../index.php/Spo_userrecommend_friend/createDropdown/?sbv=123
 		 *
 		 *    4. For listing with pre selection for multiple (select by value)
 		 *
 		 *    http://.../index.php/Spo_userrecommend_friend/createDropdown/?sbv=123,321,345&multiple=true
 		 *
 		 *    5. For listing with pre selection of select by text for single selection dropdown
 		 *
 		 *     http://.../index.php/Spo_userrecommend_friend/createDropdown/?sbt=test string&multiple=true
 		 *
 		 *    6. For xml(DHTMLX compatible) dropdown
 		 *
 		 *    http://.../index.php/Spo_userrecommend_friend/createDropdown/?format=xml
 		 *
 		 *    7. groupBy
 		 *    http://...../Spo_userrecommend_friend/createDropdown/?groupby=users.user_type
 		 *
 		 *	 8. groupCode
 		 *	  http://..../Spo_userrecommend_friend/createDropdown/?groupby=users.user_type&groupcodes=1=Staff,2=Admin,4=Manager
 		 *
 		 ...
 		 *
 		 *    groupBy - column to group, should be specified with the table name ex: spo_userrecommend_friend.staff_id
 		 *
	...
	public function createDropdown($data = false){
	  	...
	  	$isMultiple = null;
	  	$sort = null;
	    $sort_array = array('spo_userrecommend_friend.recommend_id' => 'ASC');
	  	$filterBy = null;
	   	$searchobj_array = null;
	   	$optionsOnly = false;
	   	$label = null;
	   	$displayClm = 'spo_userrecommend_friend.name';
	   	$groupBy = null;   
	   	$groupcodes = null;
	   	$nameTag = 'recommend_id'; 
	  	...
	  	if(is_array($data)){
	  		...
	  		if(count($_POST)>0 || count($_GET)>0){
	        	...
	        	$myParams = count($_POST)>0? $_POST:$_GET;
	  			...
	  			$sort = isset($myParams['sort'])?$myParams['sort']:'spo_userrecommend_friend.recommend_id=ASC';
	  			...
	  			$nameTag = (!empty($nameTag))?$nameTag:'recommend_id';                       
	      ...
	      $selection = 'spo_userrecommend_friend.recommend_id,spo_userrecommend_friend.user_id,spo_userrecommend_friend.name,spo_userrecommend_friend.friend_name,spo_userrecommend_friend.friend_email,spo_userrecommend_friend.friend_mobile,spo_userrecommend_friend.ip,spo_userrecommend_friend.date';
	    
	      // Fetch data from model
	  	  $dataset = $this->spo_userrecommend_friend_model->get_data(array(
