<?php
		/**
		 * This controller facilitates the access of table spo_client_preferences
		 *
		 * @package    HOF\controller\spo_client_preferences
		 * @version    1.0
		 ...
		 * @modification 
		 */
class Spo_client_preferences extends CI_Controller
{
    ...
    function __construct()
	{
            ...
            $this->load->model('spo_client_preferences_model');
		...
		$this->log_create->init();
            ...
            if($search==true){ 
                ...
                foreach((count($_POST)>0?$_POST:$_GET) as $key => $val){ 
                   ...
                   $data['dataset'] = $this->spo_client_preferences_model->get_data(array('search'=>$searchobj_array,'limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_client_preferences.bg_color'=>'Asc')));
            }else {
                   $data['dataset'] = $this->spo_client_preferences_model->get_data(array('limit' => $limit,'offset'=>$offset,'order_by'=>array('spo_client_preferences.bg_color'=>'Asc')));
            }   
            ...
            $wrapped_data['collist'] = $collist ? explode(':',$collist) : array('pref_id','client_id','so_review_st','my_so_reviews','appointments','calls','other_proofs','bg_color','color')  ;
            ...
            $wrapped_data['widths'] = $widths ? $widths_arr : array('pref_id'=>75,'client_id'=>75,'so_review_st'=>75,'my_so_reviews'=>75,'appointments'=>75,'calls'=>75,'other_proofs'=>75,'bg_color'=>75,'color'=>75) ;
		    ...
		    $wrapped_data['visibilityList'] = $visibilityList ? $visibilityList_arr : array('pref_id'=>true,'client_id'=>true,'so_review_st'=>true,'my_so_reviews'=>true,'appointments'=>true,'calls'=>true,'other_proofs'=>true,'bg_color'=>true,'color'=>true);
		    ...
		    $wrapped_data['cellStyles'] = $cellStyles ? $stylesList_arr : array('text-align:center','text-align:center','text-align:center','text-align:center','text-align:center','text-align:center','text-align:center','text-align:left','text-align:left');
	...
	function data_form($mode='add',$data = null){
		...
		if($mode != 'add'){
			...
			$row=$this->spo_client_preferences_model->get_data(array('search'=>array(array('search_field' => 'pref_id', 'search_text' => $data,'query_method'=>'equal')),'limit' => $limit,'offset'=>$offset)) ;
		...
		//$postData = $this->input->post();
			...
			$key2save = array('pref_id' =>$this->input->post('pref_id'));
			}
		}
		...
		switch ($mode){
			...
			case "add": //-----------------------------------------------------
				...
				//Check duplicates
				if ($this->input->post('bg_color')) {
					if ($this->spo_client_preferences_model->exists(array('bg_color' => $this->input->post('bg_color')))) {
						$message['msgText'] ='Sorry this record already exist!';
						$message['msgType'] ='failure';
						$message['focusField'] ='bg_color';
					
					}else{
						$msg = $this->spo_client_preferences_model->save($data2save,$key2save);
						if ($msg) {
			...
			case "edit":
				...
				if ($this->input->post('pref_id')) {
					...
					if (!$this->spo_client_preferences_model->exists(array('pref_id' => $this->input->post('pref_id')))) {
					 ...
					 $message['focusField'] ='bg_color';
						...
						//checking existance
						if ($this->spo_client_preferences_model->exists(array('bg_color' => $this->input->post('bg_color'),'pref_id !=' => $this->input->post('pref_id')))){
							$message['msgText'] ='Sorry this record already exist!';
							$message['msgType'] ='failure';
							$message['focusField'] ='bg_color';
		...
		$wrapper['mode'] = $mode;
		$this->load->view('spo_client_preferences_form',$wrapper);
	}
  	...
  	function save($action,$callback = null){
  		...
  		$save_data=array();
  		$key=array();
  		$metadata = $this->spo_client_preferences_model->getMetadata();
  		...
  		if($this->AIR_call_flag){
  			...
  			*/  			  			
  			if($this->AR_Utilities->request_method == 'get'){
  				...
  				$added=$this->spo_client_preferences_model->save($save_data,$key);  				
    ...
    function key_validate($action,$key) {
	    ...
	    $key_exist=$this->spo_client_preferences_model->exists($key);
	...
	function delete($String)
	{
		...
		return($this->spo_client_preferences_model->delete($delete));
	}
  	...
  	function loadForm($editable=null,$file='spo_client_preferences_form') {
 	...
 	function loadWB($file='spo_client_preferences_wb'){
	    ...
	    /**
 		 ...
 		 *
 		 *    http://.../index.php/Spo_client_preferences/createDropdown/?sbv=null&sbt=null&format=html&multiple=false&sort=asc&filterby=status_equal=1
 		 *
 		 *
 		 *    1. For normal listing:
 		 *
 		 *    http://.../index.php/Spo_client_preferences/createDropdown
 		 *
 		 *    2. For listing with multiple selection
 		 *
 		 *    http://.../index.php/Spo_client_preferences/createDropdown/?multiple=true
 		 *
 		 *    3. For listing with pre selection (select by value) for single selection drop down
 		 *
 		 *    http://.../index.php/Spo_client_preferences/createDropdown/?sbv=123
 		 *
 		 *    4. For listing with pre selection for multiple (select by value)
 		 *
 		 *    http://.../index.php/Spo_client_preferences/createDropdown/?sbv=123,321,345&multiple=true
 		 *
 		 *    5. For listing with pre selection of select by text for single selection dropdown
 		 *
 		 *     http://.../index.php/Spo_client_preferences/createDropdown/?sbt=test string&multiple=true
 		 *
 		 *    6. For xml(DHTMLX compatible) dropdown
 		 *
 		 *    http://.../index.php/Spo_client_preferences/createDropdown/?format=xml
 		 *
 		 *    7. groupBy
 		 *    http://...../Spo_client_preferences/createDropdown/?groupby=users.user_type
 		 *
 		 *	 8. groupCode
 		 *	  http://..../Spo_client_preferences/createDropdown/?groupby=users.user_type&groupcodes=1=Staff,2=Admin,4=Manager
 		 *
 		 ...
 		 *
 		 *    groupBy - column to group, should be specified with the table name ex: spo_client_preferences.staff_id
 		 *
	...
	public function createDropdown($data = false){
	  	...
	  	$isMultiple = null;
	  	$sort = null;
	    $sort_array = array('spo_client_preferences.pref_id' => 'ASC');
	  	$filterBy = null;
	   	$searchobj_array = null;
	   	$optionsOnly = false;
	   	$label = null;
	   	$displayClm = 'spo_client_preferences.bg_color';
	   	$groupBy = null;   
	   	$groupcodes = null;
	   	$nameTag = 'pref_id'; 
	  	...
	  	if(is_array($data)){
	  		...
	  		if(count($_POST)>0 || count($_GET)>0){
	        	...
	        	$myParams = count($_POST)>0? $_POST:$_GET;
	  			...
	  			$sort = isset($myParams['sort'])?$myParams['sort']:'spo_client_preferences.pref_id=ASC';
	  			...
	  			$nameTag = (!empty($nameTag))?$nameTag:'pref_id';                       
