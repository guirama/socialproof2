<?php
		/**
		 * This is model developed to maintain table spo_hof
		 *
		 * @package    HOF\models\spo_hof
		 * @version    1.0
		 ...
		 *  @modification 	 
		 */
class  Spo_hof_model extends CI_Model
{
	/**
...
$this->metadata_array['PK']= 'hof_id';

$this->metadata_array['hof_id']['header']= '';

$this->metadata_array['hof_id']['type']= 'int';

$this->metadata_array['hof_id']['width']='11';
...
$this->metadata_array['dfm_id']['header']= '';

$this->metadata_array['dfm_id']['type']= 'int';

$this->metadata_array['dfm_id']['width']='11';

		
		$metadata = $this->db->field_data('spo_hof') ;
	...
	function exists_column($column){
		return $this->db->field_exists($column,'spo_hof');
	}
	...
	private function getHeader($data){
		...
		$selection =  isset($data['selection'])? $data['selection']: 'spo_hof.hof_id,spo_hof.user_id,spo_hof.gmr_id,spo_hof.dfm_id' ;
	...
	function set_query($data,$limit_enable){
		...
		$search_filter="";
		$data['selection'] =  isset($data['selection'])? $data['selection']: 'spo_hof.hof_id,spo_hof.user_id,spo_hof.gmr_id,spo_hof.dfm_id';
	...
	function delete($pks)
	{
 	...
 	public function getMetadata() {
        ...
        /**
         * Function get_hof()
         * provide hof linkage data for a given client id
         * @param int $user_id
         * @return result set array
         */
        public function get_hof($user_id)
        {
            $this->db->where('user_id',$user_id);
            $q = $this->db->get('spo_hof');
           // echo $this->db->last_query(); 
            return $q->result_array();
        }

        /**
         * Function update_linkages()
         * Clears out existing linkage records and recreate new linkages
         * @param int GMR Id
         * @param int DFM Id
         * @param int $user_id
         * @return void
         */        
        public function update_linkages($gmr_id,$dfm_id, $user_id)
        {
            $this->db->delete('spo_hof',array('user_id'=>$user_id));
           // echo $this->db->last_query();
            $idata = array('user_id'=>$user_id,'gmr_id'=>$gmr_id,'dfm_id'=>$dfm_id);
            $this->db->insert('spo_hof',$idata);
            //echo $this->db->last_query();
        }
}
?>
