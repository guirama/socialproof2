<?php
		/**
		 * This is model developed to maintain table spo_termmeta
		 *
		 * @package    HOF\models\spo_termmeta
		 * @version    1.0
		 ...
		 *  @modification 	 
		 */
class  Spo_termmeta_model extends CI_Model
{
	/**
...
$this->metadata_array['meta_value']['width']='';
		...
		$metadata = $this->db->field_data('spo_termmeta') ;
	...
	function exists_column($column){
		return $this->db->field_exists($column,'spo_termmeta');
	}
	...
	private function getHeader($data){
		...
		$selection =  isset($data['selection'])? $data['selection']: 'spo_termmeta.meta_id,spo_termmeta.term_id,spo_termmeta.meta_key,spo_termmeta.meta_value' ;
	...
	function set_query($data,$limit_enable){
		...
		$search_filter="";
		$data['selection'] =  isset($data['selection'])? $data['selection']: 'spo_termmeta.meta_id,spo_termmeta.term_id,spo_termmeta.meta_key,spo_termmeta.meta_value';
	...
	function save(&$data_array,$key=false)
	{
		...
		if (!$key or !$this->exists($key))
			...
			if($this->db->insert('spo_termmeta'))
			{
					return true;
			...
			if($this->db->update('spo_termmeta')){
				return true;
	...
	function delete($pks)
	{
		 if (!isset($_SESSION['user_name'])){
			...
			$this->db->where($pks);
			return $this->db->delete('spo_termmeta');
		 }
 	}
