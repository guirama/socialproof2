<?php
		/**
		 * This is model developed to maintain table spo_term_relationships
		 *
		 * @package    HOF\models\spo_term_relationships
		 * @version    1.0
		 ...
		 *  @modification 	 
		 */
class  Spo_term_relationships_model extends CI_Model
{
	/**
...
$this->metadata_array['PK']= 'object_id';

$this->metadata_array['object_id']['header']= '';

$this->metadata_array['object_id']['type']= 'bigint';

$this->metadata_array['object_id']['width']='20 unsigned';
...
$this->metadata_array['term_order']['header']= '';

$this->metadata_array['term_order']['type']= 'int';

$this->metadata_array['term_order']['width']='11';

		
		$metadata = $this->db->field_data('spo_term_relationships') ;
	...
	{
		$this->db->from('spo_term_relationships');
	...
	function exists_column($column){
		return $this->db->field_exists($column,'spo_term_relationships');
	}
	...
	private function getHeader($data){
		...
		$selection =  isset($data['selection'])? $data['selection']: 'spo_term_relationships.object_id,spo_term_relationships.term_taxonomy_id,spo_term_relationships.term_order' ;
	...
	function set_query($data,$limit_enable){
		...
		$search_filter="";
		$data['selection'] =  isset($data['selection'])? $data['selection']: 'spo_term_relationships.object_id,spo_term_relationships.term_taxonomy_id,spo_term_relationships.term_order';
	...
	function save(&$data_array,$key=false)
	{
		...
		if (!$key or !$this->exists($key))
			...
			if($this->db->insert('spo_term_relationships'))
			{
					return true;
			...
			if($this->db->update('spo_term_relationships')){
				return true;
	...
	function delete($pks)
	{
		 if (!isset($_SESSION['user_name'])){
			...
			$this->db->where($pks);
			return $this->db->delete('spo_term_relationships');
		 }
 	}
