<?php
		/**
		 * This is model developed to maintain table spo_admins
		 *
		 * @package    HOF\models\spo_admins
		 * @version    1.0
		 ...
		 *  @modification 	 
		 */
class  Spo_admins_model extends CI_Model
{
	/**
...
$this->metadata_array['PK']= 'admin_id';

$this->metadata_array['admin_id']['header']= '';

$this->metadata_array['admin_id']['type']= 'int';

$this->metadata_array['admin_id']['width']='11';

$this->metadata_array['admin_name']['header']= '';

$this->metadata_array['admin_name']['type']= 'varchar';

$this->metadata_array['admin_name']['width']='255';

$this->metadata_array['username']['header']= '';

$this->metadata_array['username']['type']= 'varchar';

$this->metadata_array['username']['width']='255';
...
$this->metadata_array['email']['width']='255';
		...
		$metadata = $this->db->field_data('spo_admins') ;
	...
	function exists_column($column){
		return $this->db->field_exists($column,'spo_admins');
	}
	...
	private function getHeader($data){
		...
		$selection =  isset($data['selection'])? $data['selection']: 'spo_admins.admin_id,spo_admins.admin_name,spo_admins.username,spo_admins.password,spo_admins.email' ;
	...
	function set_query($data,$limit_enable){
		...
		$search_filter="";
		$data['selection'] =  isset($data['selection'])? $data['selection']: 'spo_admins.admin_id,spo_admins.admin_name,spo_admins.username,spo_admins.password,spo_admins.email';
	...
	function delete($pks)
	{
		 if (!isset($_SESSION['user_name'])){
			...
			$this->db->where($pks);
			return $this->db->delete('spo_admins');
		 }
 	}
