<?php
		/**
		 * This is model developed to maintain table spo_term_taxonomy
		 *
		 * @package    HOF\models\spo_term_taxonomy
		 * @version    1.0
		 ...
		 *  @modification 	 
		 */
class  Spo_term_taxonomy_model extends CI_Model
{
	/**
...
$this->metadata_array['PK']= 'term_taxonomy_id';

$this->metadata_array['term_taxonomy_id']['header']= '';

$this->metadata_array['term_taxonomy_id']['type']= 'bigint';

$this->metadata_array['term_taxonomy_id']['width']='20 unsigned';

$this->metadata_array['term_id']['header']= '';

$this->metadata_array['term_id']['type']= 'bigint';

$this->metadata_array['term_id']['width']='20 unsigned';

$this->metadata_array['taxonomy']['header']= '';

$this->metadata_array['taxonomy']['type']= 'varchar';

$this->metadata_array['taxonomy']['width']='32';

$this->metadata_array['description']['header']= '';

$this->metadata_array['description']['type']= 'longtext';

$this->metadata_array['description']['width']='';

$this->metadata_array['parent']['header']= '';

$this->metadata_array['parent']['type']= 'bigint';

$this->metadata_array['parent']['width']='20 unsigned';

$this->metadata_array['count']['header']= '';

$this->metadata_array['count']['type']= 'bigint';

$this->metadata_array['count']['width']='20';

		
		$metadata = $this->db->field_data('spo_term_taxonomy') ;
	...
	{
		$this->db->from('spo_term_taxonomy');
	...
	function exists_column($column){
		return $this->db->field_exists($column,'spo_term_taxonomy');
	}
	...
	private function getHeader($data){
		...
		$selection =  isset($data['selection'])? $data['selection']: 'spo_term_taxonomy.term_taxonomy_id,spo_term_taxonomy.term_id,spo_term_taxonomy.taxonomy,spo_term_taxonomy.description,spo_term_taxonomy.parent,spo_term_taxonomy.count' ;
	...
	function set_query($data,$limit_enable){
		...
		$search_filter="";
		$data['selection'] =  isset($data['selection'])? $data['selection']: 'spo_term_taxonomy.term_taxonomy_id,spo_term_taxonomy.term_id,spo_term_taxonomy.taxonomy,spo_term_taxonomy.description,spo_term_taxonomy.parent,spo_term_taxonomy.count';
	...
	function save(&$data_array,$key=false)
	{
		...
		if (!$key or !$this->exists($key))
			...
			if($this->db->insert('spo_term_taxonomy'))
			{
					return true;
			...
			if($this->db->update('spo_term_taxonomy')){
				return true;
	...
	function delete($pks)
	{
		 if (!isset($_SESSION['user_name'])){
			...
			$this->db->where($pks);
			return $this->db->delete('spo_term_taxonomy');
		 }
 	}
