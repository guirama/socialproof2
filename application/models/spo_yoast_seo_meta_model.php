<?php
		/**
		 * This is model developed to maintain table spo_yoast_seo_meta
		 *
		 * @package    HOF\models\spo_yoast_seo_meta
		 * @version    1.0
		 ...
		 *  @modification 	 
		 */
class  Spo_yoast_seo_meta_model extends CI_Model
{
	/**
...
$this->metadata_array['internal_link_count']['header']= '';

$this->metadata_array['internal_link_count']['type']= 'int';

$this->metadata_array['internal_link_count']['width']='10 unsigned';

$this->metadata_array['incoming_link_count']['header']= '';

$this->metadata_array['incoming_link_count']['type']= 'int';

$this->metadata_array['incoming_link_count']['width']='10 unsigned';

		
		$metadata = $this->db->field_data('spo_yoast_seo_meta') ;
	...
	{
		$this->db->from('spo_yoast_seo_meta');
	...
	function exists_column($column){
		return $this->db->field_exists($column,'spo_yoast_seo_meta');
	}
	...
	private function getHeader($data){
		...
		$selection =  isset($data['selection'])? $data['selection']: 'spo_yoast_seo_meta.object_id,spo_yoast_seo_meta.internal_link_count,spo_yoast_seo_meta.incoming_link_count' ;
	...
	function set_query($data,$limit_enable){
		...
		$search_filter="";
		$data['selection'] =  isset($data['selection'])? $data['selection']: 'spo_yoast_seo_meta.object_id,spo_yoast_seo_meta.internal_link_count,spo_yoast_seo_meta.incoming_link_count';
	...
	function save(&$data_array,$key=false)
	{
		...
		if (!$key or !$this->exists($key))
			...
			if($this->db->insert('spo_yoast_seo_meta'))
			{
					return true;
			...
			if($this->db->update('spo_yoast_seo_meta')){
				return true;
	...
	function delete($pks)
	{
		 if (!isset($_SESSION['user_name'])){
			...
			$this->db->where($pks);
			return $this->db->delete('spo_yoast_seo_meta');
		 }
 	}
