<?php
		/**
		 * This is model developed to maintain table spo_options
		 *
		 * @package    HOF\models\spo_options
		 * @version    1.0
		 ...
		 *  @modification 	 
		 */
class  Spo_options_model extends CI_Model
{
	/**
...
$this->metadata_array['PK']= 'option_id';

$this->metadata_array['option_id']['header']= '';

$this->metadata_array['option_id']['type']= 'bigint';

$this->metadata_array['option_id']['width']='20 unsigned';

$this->metadata_array['option_name']['header']= '';

$this->metadata_array['option_name']['type']= 'varchar';

$this->metadata_array['option_name']['width']='191';

$this->metadata_array['option_value']['header']= '';

$this->metadata_array['option_value']['type']= 'longtext';

$this->metadata_array['option_value']['width']='';

$this->metadata_array['autoload']['header']= '';

$this->metadata_array['autoload']['type']= 'varchar';

$this->metadata_array['autoload']['width']='20';

		
		$metadata = $this->db->field_data('spo_options') ;
	...
	function exists_column($column){
		return $this->db->field_exists($column,'spo_options');
	}
	...
	private function getHeader($data){
		...
		$selection =  isset($data['selection'])? $data['selection']: 'spo_options.option_id,spo_options.option_name,spo_options.option_value,spo_options.autoload' ;
	...
	function set_query($data,$limit_enable){
		...
		$search_filter="";
		$data['selection'] =  isset($data['selection'])? $data['selection']: 'spo_options.option_id,spo_options.option_name,spo_options.option_value,spo_options.autoload';
	...
	function delete($pks)
	{
		 if (!isset($_SESSION['user_name'])){
			...
			$this->db->where($pks);
			return $this->db->delete('spo_options');
		 }
 	}
