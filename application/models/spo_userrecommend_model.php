<?php
		/**
		 * This is model developed to maintain table spo_userrecommend
		 *
		 * @package    HOF\models\spo_userrecommend
		 * @version    1.0
		 ...
		 *  @modification 	 
		 */
class  Spo_userrecommend_model extends CI_Model
{
	/**
...
$this->metadata_array['date']['width']='';
		...
		$metadata = $this->db->field_data('spo_userrecommend') ;
	...
	{
		$this->db->from('spo_userrecommend');
	...
	function exists_column($column){
		return $this->db->field_exists($column,'spo_userrecommend');
	}
	...
	private function getHeader($data){
		...
		$selection =  isset($data['selection'])? $data['selection']: 'spo_userrecommend.recommend_id,spo_userrecommend.user_id,spo_userrecommend.name,spo_userrecommend.ip,spo_userrecommend.date' ;
	...
	function set_query($data,$limit_enable){
		...
		$search_filter="";
		$data['selection'] =  isset($data['selection'])? $data['selection']: 'spo_userrecommend.recommend_id,spo_userrecommend.user_id,spo_userrecommend.name,spo_userrecommend.ip,spo_userrecommend.date';
	...
	function save(&$data_array,$key=false)
	{
		...
		if (!$key or !$this->exists($key))
			...
			if($this->db->insert('spo_userrecommend'))
			{
					return true;
			...
			if($this->db->update('spo_userrecommend')){
				return true;
	...
	function delete($pks)
	{
		 if (!isset($_SESSION['user_name'])){
			...
			$this->db->where($pks);
			return $this->db->delete('spo_userrecommend');
		 }
 	}
