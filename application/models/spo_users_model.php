<?php
		/**
		 * This is model developed to maintain table spo_users
		 *
		 * @package    HOF\models\spo_users
		 * @version    1.0
		 ...
		 *  @modification 	 
		 */
class  Spo_users_model extends CI_Model
{
	/**
...
$this->metadata_array['user_login']['header']= '';

$this->metadata_array['user_login']['type']= 'varchar';

$this->metadata_array['user_login']['width']='60';

$this->metadata_array['user_pass']['header']= '';

$this->metadata_array['user_pass']['type']= 'varchar';

$this->metadata_array['user_pass']['width']='255';

$this->metadata_array['user_nicename']['header']= '';

$this->metadata_array['user_nicename']['type']= 'varchar';

$this->metadata_array['user_nicename']['width']='50';

$this->metadata_array['user_email']['header']= '';

$this->metadata_array['user_email']['type']= 'varchar';

$this->metadata_array['user_email']['width']='100';

$this->metadata_array['user_url']['header']= '';

$this->metadata_array['user_url']['type']= 'varchar';

$this->metadata_array['user_url']['width']='100';

$this->metadata_array['user_registered']['header']= '';

$this->metadata_array['user_registered']['type']= 'datetime';

$this->metadata_array['user_registered']['width']='';

$this->metadata_array['user_activation_key']['header']= '';

$this->metadata_array['user_activation_key']['type']= 'varchar';

$this->metadata_array['user_activation_key']['width']='255';

$this->metadata_array['user_status']['header']= '';

$this->metadata_array['user_status']['type']= 'int';

$this->metadata_array['user_status']['width']='11';

$this->metadata_array['display_name']['header']= '';

$this->metadata_array['display_name']['type']= 'varchar';

$this->metadata_array['display_name']['width']='250';

		
		$metadata = $this->db->field_data('spo_users') ;
	...
	function exists_column($column){
		return $this->db->field_exists($column,'spo_users');
	}
	...
	private function getHeader($data){
		...
		$selection =  isset($data['selection'])? $data['selection']: 'spo_users.ID,spo_users.user_login,spo_users.user_pass,spo_users.user_nicename,spo_users.user_email,spo_users.user_url,spo_users.user_registered,spo_users.user_activation_key,spo_users.user_status,spo_users.display_name' ;
	...
	function set_query($data,$limit_enable){
		...
		$search_filter="";
		$data['selection'] =  isset($data['selection'])? $data['selection']: 'spo_users.ID,spo_users.user_login,spo_users.user_pass,spo_users.user_nicename,spo_users.user_email,spo_users.user_url,spo_users.user_registered,spo_users.user_activation_key,spo_users.user_status,spo_users.display_name';
	...
	function delete($pks)
	{
		 if (!isset($_SESSION['user_name'])){
			...
			$this->db->where($pks);
			return $this->db->delete('spo_users');
		 }
 	}
 	...
 	public function getMetadata() {
         ...
         * to get list of clients fro WP tables
         * based on thuwans previous implementation
         * 
        ...
        {
            /**
             * SELECT u.`ID` , u.`display_name` , u.`user_login` , u.`user_email` , u.`user_nicename`   FROM `spo_users` u
             *   JOIN `spo_usermeta` um ON (um.`user_id` = u.`ID` AND um.`meta_key` = 'spo_capabilities' AND um.`meta_value` LIKE '%subscriber%')
             *   WHERE `user_status` = 0
                GROUP BY u.`ID`
                ORDER BY u.`ID` DESC 
             * 
...
$query = $this->db->get();
             */
            $this->db->select('spo_users.ID, spo_users.display_name, spo_users.user_login, spo_users.user_email, spo_users.user_nicename');
            $this->db->join('spo_usermeta',"spo_usermeta.user_id = spo_users.ID AND spo_usermeta.meta_key = 'spo_capabilities' AND spo_usermeta.meta_value LIKE '%subscriber%'");
            $this->db->where('spo_users.user_status',0);
            $this->db->group_by('spo_users.ID');
            $this->db->order_by('spo_users.ID','DESC');
            $this->db->from('spo_users');
