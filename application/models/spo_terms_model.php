<?php
		/**
		 * This is model developed to maintain table spo_terms
		 *
		 * @package    HOF\models\spo_terms
		 * @version    1.0
		 ...
		 *  @modification 	 
		 */
class  Spo_terms_model extends CI_Model
{
	/**
...
$this->metadata_array['PK']= 'term_id';
...
$this->metadata_array['name']['width']='200';

$this->metadata_array['slug']['header']= '';

$this->metadata_array['slug']['type']= 'varchar';

$this->metadata_array['slug']['width']='200';

$this->metadata_array['term_group']['header']= '';

$this->metadata_array['term_group']['type']= 'bigint';

$this->metadata_array['term_group']['width']='10';

		
		$metadata = $this->db->field_data('spo_terms') ;
	...
	function exists_column($column){
		return $this->db->field_exists($column,'spo_terms');
	}
	...
	private function getHeader($data){
		...
		$selection =  isset($data['selection'])? $data['selection']: 'spo_terms.term_id,spo_terms.name,spo_terms.slug,spo_terms.term_group' ;
	...
	function set_query($data,$limit_enable){
		...
		$search_filter="";
		$data['selection'] =  isset($data['selection'])? $data['selection']: 'spo_terms.term_id,spo_terms.name,spo_terms.slug,spo_terms.term_group';
	...
	function delete($pks)
	{
		 if (!isset($_SESSION['user_name'])){
			...
			$this->db->where($pks);
			return $this->db->delete('spo_terms');
		 }
 	}
