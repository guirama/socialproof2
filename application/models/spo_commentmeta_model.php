<?php
		/**
		 * This is model developed to maintain table spo_commentmeta
		 *
		 * @package    HOF\models\spo_commentmeta
		 * @version    1.0
		 ...
		 *  @modification 	 
		 */
class  Spo_commentmeta_model extends CI_Model
{
	/**
...
$this->metadata_array['PK']= 'meta_id';

$this->metadata_array['meta_id']['header']= '';

$this->metadata_array['meta_id']['type']= 'bigint';

$this->metadata_array['meta_id']['width']='20 unsigned';
...
$this->metadata_array['meta_key']['header']= '';

$this->metadata_array['meta_key']['type']= 'varchar';

$this->metadata_array['meta_key']['width']='255';

$this->metadata_array['meta_value']['header']= '';

$this->metadata_array['meta_value']['type']= 'longtext';

$this->metadata_array['meta_value']['width']='';

		
		$metadata = $this->db->field_data('spo_commentmeta') ;
	...
	function exists_column($column){
		return $this->db->field_exists($column,'spo_commentmeta');
	}
	...
	private function getHeader($data){
		...
		$selection =  isset($data['selection'])? $data['selection']: 'spo_commentmeta.meta_id,spo_commentmeta.comment_id,spo_commentmeta.meta_key,spo_commentmeta.meta_value' ;
	...
	function set_query($data,$limit_enable){
		...
		$search_filter="";
		$data['selection'] =  isset($data['selection'])? $data['selection']: 'spo_commentmeta.meta_id,spo_commentmeta.comment_id,spo_commentmeta.meta_key,spo_commentmeta.meta_value';
	...
	function save(&$data_array,$key=false)
	{
		...
		if (!$key or !$this->exists($key))
			...
			if($this->db->insert('spo_commentmeta'))
			{
					return true;
			...
			if($this->db->update('spo_commentmeta')){
				return true;
	...
	function delete($pks)
	{
		 if (!isset($_SESSION['user_name'])){
			...
			$this->db->where($pks);
			return $this->db->delete('spo_commentmeta');
		 }
 	}
