<?php
		/**
		 * This is model developed to maintain table spo_postmeta
		 *
		 * @package    HOF\models\spo_postmeta
		 * @version    1.0
		 ...
		 *  @modification 	 
		 */
class  Spo_postmeta_model extends CI_Model
{
	/**
...
$this->metadata_array['meta_value']['width']='';
		...
		$metadata = $this->db->field_data('spo_postmeta') ;
	...
	function exists_column($column){
		return $this->db->field_exists($column,'spo_postmeta');
	}
	...
	private function getHeader($data){
		...
		$selection =  isset($data['selection'])? $data['selection']: 'spo_postmeta.meta_id,spo_postmeta.post_id,spo_postmeta.meta_key,spo_postmeta.meta_value' ;
	...
	function set_query($data,$limit_enable){
		...
		$search_filter="";
		$data['selection'] =  isset($data['selection'])? $data['selection']: 'spo_postmeta.meta_id,spo_postmeta.post_id,spo_postmeta.meta_key,spo_postmeta.meta_value';
	...
	function save(&$data_array,$key=false)
	{
		...
		if (!$key or !$this->exists($key))
			...
			if($this->db->insert('spo_postmeta'))
			{
					return true;
			...
			if($this->db->update('spo_postmeta')){
				return true;
	...
	function delete($pks)
	{
		 if (!isset($_SESSION['user_name'])){
			...
			$this->db->where($pks);
			return $this->db->delete('spo_postmeta');
		 }
 	}
