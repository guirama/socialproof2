<?php
		/**
		 * This is model developed to maintain table spo_hof_banners
		 *
		 * @package    HOF\models\spo_hof_banners
		 * @version    1.0
		 ...
		 *  @modification 	 
		 */
class  Spo_hof_banners_model extends CI_Model
{
	/**
...
$this->metadata_array['PK']= 'banner_id';

$this->metadata_array['banner_id']['header']= '';

$this->metadata_array['banner_id']['type']= 'int';

$this->metadata_array['banner_id']['width']='11';
...
$this->metadata_array['hof_review_id']['header']= '';

$this->metadata_array['hof_review_id']['type']= 'varchar';

$this->metadata_array['hof_review_id']['width']='255';

$this->metadata_array['url']['header']= '';

$this->metadata_array['url']['type']= 'varchar';

$this->metadata_array['url']['width']='255';

$this->metadata_array['template_id']['header']= '';

$this->metadata_array['template_id']['type']= 'varchar';

$this->metadata_array['template_id']['width']='255';
...
$this->metadata_array['dfm_date']['header']= '';

$this->metadata_array['dfm_date']['type']= 'date';

$this->metadata_array['dfm_date']['width']='';

$this->metadata_array['tagline']['header']= '';

$this->metadata_array['tagline']['type']= 'text';

$this->metadata_array['tagline']['width']='';

$this->metadata_array['banner_created_on']['header']= '';

$this->metadata_array['banner_created_on']['type']= 'timestamp';

$this->metadata_array['banner_created_on']['width']='';

		
		$metadata = $this->db->field_data('spo_hof_banners') ;
	...
	function exists_column($column){
		return $this->db->field_exists($column,'spo_hof_banners');
	}
	...
	private function getHeader($data){
		...
		$selection =  isset($data['selection'])? $data['selection']: 'spo_hof_banners.banner_id,spo_hof_banners.user_id,spo_hof_banners.hof_review_id,spo_hof_banners.url,spo_hof_banners.template_id,spo_hof_banners.created_on,spo_hof_banners.dfm_date,spo_hof_banners.tagline,spo_hof_banners.banner_created_on' ;
	...
	function set_query($data,$limit_enable){
		...
		$search_filter="";
		$data['selection'] =  isset($data['selection'])? $data['selection']: 'spo_hof_banners.banner_id,spo_hof_banners.user_id,spo_hof_banners.hof_review_id,spo_hof_banners.url,spo_hof_banners.template_id,spo_hof_banners.created_on,spo_hof_banners.dfm_date,spo_hof_banners.tagline,spo_hof_banners.banner_created_on';
	...
	function save(&$data_array,$key=false)
	{
		...
		if (!$key or !$this->exists($key))
			...
			if($this->db->insert('spo_hof_banners'))
			{
					return true;
			...
			if($this->db->update('spo_hof_banners')){
				return true;
	...
	function delete($pks)
	{
		 if (!isset($_SESSION['user_name'])){
			...
			$this->db->where($pks);
			return $this->db->delete('spo_hof_banners');
		 }
 	}
