<?php
		/**
		 * This is model developed to maintain table spo_custom_reviews
		 *
		 * @package    HOF\models\spo_custom_reviews
		 * @version    1.0
		 ...
		 *  @modification 	 
		 */
class  Spo_custom_reviews_model extends CI_Model
{
	/**
...
$this->metadata_array['reviewer_phone']['header']= '';

$this->metadata_array['reviewer_phone']['type']= 'varchar';

$this->metadata_array['reviewer_phone']['width']='255';
...
$this->metadata_array['created_on']['width']='';
		...
		$metadata = $this->db->field_data('spo_custom_reviews') ;
	...
	{
		$this->db->from('spo_custom_reviews');
	...
	function exists_column($column){
		return $this->db->field_exists($column,'spo_custom_reviews');
	}
	...
	function set_query($data,$limit_enable){
		...
		$search_filter="";
		$data['selection'] =  isset($data['selection'])? $data['selection']: 'spo_custom_reviews.review_id,spo_custom_reviews.user_id,spo_custom_reviews.rating,spo_custom_reviews.reviewer_name,spo_custom_reviews.reviewer_email,spo_custom_reviews.reviewer_phone,spo_custom_reviews.review,spo_custom_reviews.provider_name,spo_custom_reviews.profile_link,spo_custom_reviews.created_on';
	...
	function save(&$data_array,$key=false)
	{
		...
		if (!$key or !$this->exists($key))
			...
			if($this->db->insert('spo_custom_reviews'))
			{
					return true;
			...
			if($this->db->update('spo_custom_reviews')){
				return true;
	...
	function delete($pks)
	{
		 if (!isset($_SESSION['user_name'])){
			...
			$this->db->where($pks);
			return $this->db->delete('spo_custom_reviews');
		 }
 	}
