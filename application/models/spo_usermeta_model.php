<?php
		/**
		 * This is model developed to maintain table spo_usermeta
		 *
		 * @package    HOF\models\spo_usermeta
		 * @version    1.0
		 ...
		 *  @modification 	 
		 */
class  Spo_usermeta_model extends CI_Model
{
	/**
...
$this->metadata_array['PK']= 'umeta_id';

$this->metadata_array['umeta_id']['header']= '';

$this->metadata_array['umeta_id']['type']= 'bigint';

$this->metadata_array['umeta_id']['width']='20 unsigned';
...
$this->metadata_array['meta_value']['width']='';
		...
		$metadata = $this->db->field_data('spo_usermeta') ;
	...
	function exists_column($column){
		return $this->db->field_exists($column,'spo_usermeta');
	}
	...
	private function getHeader($data){
		...
		$selection =  isset($data['selection'])? $data['selection']: 'spo_usermeta.umeta_id,spo_usermeta.user_id,spo_usermeta.meta_key,spo_usermeta.meta_value' ;
	...
	function set_query($data,$limit_enable){
		...
		$search_filter="";
		$data['selection'] =  isset($data['selection'])? $data['selection']: 'spo_usermeta.umeta_id,spo_usermeta.user_id,spo_usermeta.meta_key,spo_usermeta.meta_value';
	...
	function save(&$data_array,$key=false)
	{
		...
		if (!$key or !$this->exists($key))
			...
			if($this->db->insert('spo_usermeta'))
			{
					return true;
			...
			if($this->db->update('spo_usermeta')){
				return true;
	...
	function delete($pks)
	{
		 if (!isset($_SESSION['user_name'])){
			...
			$this->db->where($pks);
			return $this->db->delete('spo_usermeta');
		 }
 	}
