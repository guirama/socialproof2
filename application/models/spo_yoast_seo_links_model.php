<?php
		/**
		 * This is model developed to maintain table spo_yoast_seo_links
		 *
		 * @package    HOF\models\spo_yoast_seo_links
		 * @version    1.0
		 ...
		 *  @modification 	 
		 */
class  Spo_yoast_seo_links_model extends CI_Model
{
	/**
...
$this->metadata_array['post_id']['header']= '';

$this->metadata_array['post_id']['type']= 'bigint';

$this->metadata_array['post_id']['width']='20 unsigned';

$this->metadata_array['target_post_id']['header']= '';

$this->metadata_array['target_post_id']['type']= 'bigint';

$this->metadata_array['target_post_id']['width']='20 unsigned';

$this->metadata_array['type']['header']= '';

$this->metadata_array['type']['type']= 'varchar';

$this->metadata_array['type']['width']='8';

		
		$metadata = $this->db->field_data('spo_yoast_seo_links') ;
	...
	{
		$this->db->from('spo_yoast_seo_links');
	...
	function exists_column($column){
		return $this->db->field_exists($column,'spo_yoast_seo_links');
	}
	...
	private function getHeader($data){
		...
		$selection =  isset($data['selection'])? $data['selection']: 'spo_yoast_seo_links.id,spo_yoast_seo_links.url,spo_yoast_seo_links.post_id,spo_yoast_seo_links.target_post_id,spo_yoast_seo_links.type' ;
	...
	function set_query($data,$limit_enable){
		...
		$search_filter="";
		$data['selection'] =  isset($data['selection'])? $data['selection']: 'spo_yoast_seo_links.id,spo_yoast_seo_links.url,spo_yoast_seo_links.post_id,spo_yoast_seo_links.target_post_id,spo_yoast_seo_links.type';
	...
	function save(&$data_array,$key=false)
	{
		...
		if (!$key or !$this->exists($key))
			...
			if($this->db->insert('spo_yoast_seo_links'))
			{
					return true;
			...
			if($this->db->update('spo_yoast_seo_links')){
				return true;
	...
	function delete($pks)
	{
		 if (!isset($_SESSION['user_name'])){
			...
			$this->db->where($pks);
			return $this->db->delete('spo_yoast_seo_links');
		 }
 	}
