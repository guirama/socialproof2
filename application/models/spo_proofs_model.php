<?php
		/**
		 * This is model developed to maintain table spo_proofs
		 *
		 * @package    HOF\models\spo_proofs
		 * @version    1.0
		 ...
		 *  @modification 	 
		 */
class  Spo_proofs_model extends CI_Model
{
	/**
...
$this->metadata_array['PK']= 'proof_id';

$this->metadata_array['proof_id']['header']= '';

$this->metadata_array['proof_id']['type']= 'int';

$this->metadata_array['proof_id']['width']='11';
...
$this->metadata_array['image']['header']= '';

$this->metadata_array['image']['type']= 'varchar';

$this->metadata_array['image']['width']='255';
...
$this->metadata_array['status']['header']= '';

$this->metadata_array['status']['type']= 'tinyint';

$this->metadata_array['status']['width']='1';

		
		$metadata = $this->db->field_data('spo_proofs') ;
	...
	function exists_column($column){
		return $this->db->field_exists($column,'spo_proofs');
	}
	...
	private function getHeader($data){
		...
		$selection =  isset($data['selection'])? $data['selection']: 'spo_proofs.proof_id,spo_proofs.client_id,spo_proofs.image,spo_proofs.comment,spo_proofs.status' ;
	...
	function set_query($data,$limit_enable){
		...
		$search_filter="";
		$data['selection'] =  isset($data['selection'])? $data['selection']: 'spo_proofs.proof_id,spo_proofs.client_id,spo_proofs.image,spo_proofs.comment,spo_proofs.status';
	...
	function delete($pks)
	{
		 if (!isset($_SESSION['user_name'])){
			...
			$this->db->where($pks);
			return $this->db->delete('spo_proofs');
		 }
 	}
