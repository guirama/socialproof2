<?php
		/**
		 * This is model developed to maintain table spo_client_reviews
		 *
		 * @package    HOF\models\spo_client_reviews
		 * @version    1.0
		 ...
		 *  @modification 	 
		 */
class  Spo_client_reviews_model extends CI_Model
{
	/**
...
$this->metadata_array['review_id']['type']= 'varchar';

$this->metadata_array['review_id']['width']='255';
...
$this->metadata_array['user_name']['header']= '';

$this->metadata_array['user_name']['type']= 'varchar';

$this->metadata_array['user_name']['width']='255';

$this->metadata_array['comment']['header']= '';

$this->metadata_array['comment']['type']= 'text';

$this->metadata_array['comment']['width']='';
...
$this->metadata_array['created_on']['width']='';
		...
		$metadata = $this->db->field_data('spo_client_reviews') ;
	...
	{
		$this->db->from('spo_client_reviews');
	...
	function exists_column($column){
		return $this->db->field_exists($column,'spo_client_reviews');
	}
	...
	private function getHeader($data){
		...
		$selection =  isset($data['selection'])? $data['selection']: 'spo_client_reviews.id,spo_client_reviews.review_id,spo_client_reviews.user_id,spo_client_reviews.rating,spo_client_reviews.user_name,spo_client_reviews.comment,spo_client_reviews.provider_name,spo_client_reviews.profile_link,spo_client_reviews.created_on' ;
	...
	function set_query($data,$limit_enable){
		...
		$search_filter="";
		$data['selection'] =  isset($data['selection'])? $data['selection']: 'spo_client_reviews.id,spo_client_reviews.review_id,spo_client_reviews.user_id,spo_client_reviews.rating,spo_client_reviews.user_name,spo_client_reviews.comment,spo_client_reviews.provider_name,spo_client_reviews.profile_link,spo_client_reviews.created_on';
	...
	function save(&$data_array,$key=false)
	{
		...
		if (!$key or !$this->exists($key))
			...
			if($this->db->insert('spo_client_reviews'))
			{
					return true;
			...
			if($this->db->update('spo_client_reviews')){
				return true;
	...
	function delete($pks)
	{
		 if (!isset($_SESSION['user_name'])){
			...
			$this->db->where($pks);
			return $this->db->delete('spo_client_reviews');
		 }
 	}
