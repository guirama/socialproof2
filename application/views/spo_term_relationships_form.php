<?php
		 ...
		 *
		 * @package			HOF\view\spo_term_relationships_form 
		 * @version			1.0
		 * @uses
		 * @see
		 ...
		 * @modification
		 */
class spo_term_relationships_form{
...
$object_id,term_taxonomy_id	= isset($object_id,term_taxonomy_id)? $object_id,term_taxonomy_id : "";
$object_id = isset($object_id)? $object_id: "";
$term_taxonomy_id = isset($term_taxonomy_id)? $term_taxonomy_id: "";
$term_order = isset($term_order)? $term_order: "";
...
$log_create->set_otherDetails(array('Mode'=>"spo_term_relationships_Form-$mode",'Data'=>$object_id,term_taxonomy_id));
$log_create->init();


$fieldset_title ="Spo_term_relationships (".ucfirst($mode).")";
?>
...
<?
//Create new form
$myForm = new form_object('spo_term_relationships_form') ;
	...
	$myForm->formOpenTag = array('method' => 'post',
		...
		$id_field = new field('object_id,term_taxonomy_id','hidden',array('value'=>$object_id,term_taxonomy_id,'required_mark' => false));
...
$field_2 = new field(
	...
	$myForm->fieldSets = array(
			'tpdetails' => array(
					...
					'fields' =>  array(
							$id_field,
							$mode_field,$form_status_field,$field_0,$field_1,$field_2
							)
					)
	);
$myForm->init();
...
function inputValidate(){
	...
	idlist = Array('object_id','term_taxonomy_id','term_order');
	iddis = Array('','','');
