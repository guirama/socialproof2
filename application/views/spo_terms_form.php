<?php
		 ...
		 *
		 * @package			HOF\view\spo_terms_form 
		 * @version			1.0
		 * @uses
		 * @see
...
$term_group = isset($term_group)? $term_group: "";
...
$log_create->set_otherDetails(array('Mode'=>"spo_terms_Form-$mode",'Data'=>$term_id));
$log_create->init();


$fieldset_title ="Spo_terms (".ucfirst($mode).")";
?>
...
<?
//Create new form
$myForm = new form_object('spo_terms_form') ;
	...
	$myForm->formOpenTag = array('method' => 'post',
		...
		$id_field = new field('term_id','hidden',array('value'=>$term_id,'required_mark' => false));
...
function inputValidate(){
	...
	idlist = Array('term_id','name','slug','term_group');
	iddis = Array('','','','');
