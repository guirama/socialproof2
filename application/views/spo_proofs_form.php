<?php
		 ...
		 *
		 * @package			HOF\view\spo_proofs_form 
		 * @version			1.0
		 * @uses
		 * @see
...
$proof_id	= isset($proof_id)? $proof_id : "";
$proof_id = isset($proof_id)? $proof_id: "";
...
$log_create->set_otherDetails(array('Mode'=>"spo_proofs_Form-$mode",'Data'=>$proof_id));
$log_create->init();


$fieldset_title ="Spo_proofs (".ucfirst($mode).")";
?>
...
<?
//Create new form
$myForm = new form_object('spo_proofs_form') ;
	...
	$myForm->formOpenTag = array('method' => 'post',
		...
		$id_field = new field('proof_id','hidden',array('value'=>$proof_id,'required_mark' => false));
...
function inputValidate(){
	...
	idlist = Array('proof_id','client_id','image','comment','status');
	iddis = Array('','','','','');
