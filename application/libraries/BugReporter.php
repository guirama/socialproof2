<?php
	/**
 	 * class.bugzillaxml.php file
 	 *
 	 * This is the bugzilla API wrapper CI library class file
 	 *
 	 * @package    CI\Libraries
 	 * @author     M.R.M.Roshan
 	 * @version    V 1.0.0
 	 */
	if (!defined('BASEPATH')) exit('No direct script access allowed');
	
	define("BUGZILLA_URL", "http://kindersigns.org/bugzilla/xmlrpc.cgi");	
	...
	*   
	* @package    CI\Libraries
	* @author    Evan Oman, John Eckhardt
	* @version    V 1.0.0
	* @created   21/1/2016
	*
	*/
...
/**
 *  This is the BugReporter class
 *  
 *  
 *  
 *  This class is written to communicate with bugzilla DB through the API class 
 *  
 *
 *  @package      CIGEN\libraries\Bugreporter
 *  @version	  1.0
 *  @uses
 *  @see
...
class BugReporter{
	...
	function __construct($product='test',$username='roshan@ekwa.com',$password='ratnapura'){
		$this->username = $username;//'roshan@ekwa.com';
		$this->password = $password;//'ratnapura';
	...
	function get_product_details(){
		...
		$bugzilla->addMember("names", $this->product, "none");		
	...
	public function login(){
		...
		$bugzilla = new BugzillaXML("User.login");
		$bugzilla->addMember("login", $this->username, "none");
		$bugzilla->addMember("password", $this->password, "none");
		$return = $bugzilla->submit(true);

		if(isset($return['faultCode']) && isset($return['faultString'])){
			echo "Bugzilla Code ". $return['faultCode']." : ". $return['faultString'];
			exit;
		}else if(isset($return['id'])){
	...
	function get_comments($bug_id = 21785){
		
		$bugzilla = new BugzillaXML("Bug.comments");		
	...
	function get_all_bugs($limit=1000,$offset=0){
		
		$bugzilla = new BugzillaXML("Bug.search");		
		$paramArr["offset"] = $offset;
	...
	function create_bug(){
		...
		if(isset($_POST['btnReportBug'])){

			$msg = null;
			$bugzilla = new BugzillaXML("Bug.create");
			
			$paramArr["product"] = $this->product;
			$paramArr["component"] = $_POST['component'];
			$paramArr["summary"] = $_POST['bugTitle'];
			$paramArr["version"] = $_POST['virsion'];
			$paramArr['bug_severity']=$_POST['severity'];
			$paramArr['op_sys'] = "All";
			$paramArr["rep_platform"] = "All";
			...
			if(isset($return['id'])){
				...
				$msg = $return["faultString"];
			}
		}
		
		$prod = $this->get_product_details();		
		$components = $prod['products'][0]['components'];
		$virsions = $prod['products'][0]['versions'];		
		?>
		
		<form method="post" action="">
		<?php echo (!empty($msg))?"<br><br>".$msg."<br><br>":null;?>
		<table>
		<tr>
		<td>Bug Summary</td><td><input type="text" value="" name="bugTitle"></td>
		</tr>
		<tr>
		<td>Component</td>
		<td>
		...
		<tr>
		<td></td><td><input type="submit" value="Report" name="btnReportBug"></td>		
		</tr>
		</table>
		</form>
		<?php 
